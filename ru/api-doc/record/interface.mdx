...
<Note>Поле подстановки не является отдельным типом Поля. Можно подставлять Поле любого типа из связанного Проекта. Тип Поля подстановки определяется исходным Полем в связанном Проекте и имеет свойство isLookup. Поля подстановки нельзя редактировать.</Note>
