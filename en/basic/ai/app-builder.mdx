...
If you're running Teable self-hosted, configure App Builder settings in **Admin Panel → AI Settings → App**.

### Required Settings

| Setting | Description |
|---------|-------------|
| **v0 API Key** | Get from [v0.dev/chat/settings/keys](https://v0.dev/chat/settings/keys) |
...
To use custom domains for published apps:

1. Purchase a domain from [Vercel Domains](https://vercel.com/domains)
2. Enter the domain in **Custom Domain** field
3. Add your **Vercel API Token** from [Vercel Settings](https://vercel.com/account/settings/tokens)

<Note>
Apps are deployed to Vercel's cloud infrastructure, not your Teable server. Your existing reverse proxy (Nginx, Coolify) or Cloudflare tunnels cannot route traffic to published apps. Path-based URLs (e.g., `yourdomain.com/app/xxx`) are also not supported.
</Note>
