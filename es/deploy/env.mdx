...
| BACKEND_SIGNIN_VERIFICATION_EXPIRES_IN     | Vigencia del código de verificación de inicio de sesión. Si no se define, se usa BACKEND_EMAIL_CODE_EXPIRES_IN | 10m              | -    | 10m                                              |
| BACKEND_SIGNIN_VERIFICATION_MAX_ATTEMPTS   | Intentos fallidos permitidos por código de inicio de sesión; a partir de ahí el código se descarta y hay que solicitar otro | 5                | -    | 5                                                |
...
| BACKEND_MAIL_INVITE_SPACE_NAME_MAX_LENGTH  | Longitud máxima del nombre del Espacio o el Proyecto en los asuntos de los correos de invitación. Un valor vacío o 0 desactiva el truncamiento | - | - | 32 |
...
| TABLE_LIMIT_TABLES_PER_BASE_MAX            | Número máximo de Tablas por Proyecto                                                   | 1000            | -        | 1000                                              |
...
| V2_TABLE_QUERY_OPS_ENABLED                 | Indica si se activan las operaciones de consulta de Tablas; mientras estén desactivadas, la página **Operaciones de consulta de Tablas** no tiene datos. Reinicia tras el cambio        | false           | -        | true                                              |
...
| PUBLIC_DATABASE_PROXY                      | Dirección de la base de datos accesible externamente (host:puerto) utilizada en las credenciales que genera el panel «Conexión de base de datos» del Proyecto; establécela para activar el acceso SQL externo de solo lectura | -               | -        | db.example.com:42345                              |
...
| BIG_TRANSACTION_TIMEOUT                    | Tiempo de espera (ms) para transacciones internas grandes, como la exportación de Proyectos grandes | 600000          | -        | 1200000                                           |
