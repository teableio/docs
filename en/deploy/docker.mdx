...
**Docker Compose is for evaluation and small teams only** (< 100 users, no high availability).
For production deployments, first read the [Self-Hosted Overview](/en/deploy/architecture), then follow [Kubernetes](/en/deploy/k8s) or a cloud-specific guide.
</Callout>

## Docker Compose Deployment

**Recommended Scale**: 0-100 users
...
For production deployments, Teable is strongly recommended to use **S3-compatible object storage** (S3 / MinIO / other S3-compatible services) for better durability and scaling.
