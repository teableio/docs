...
Hücredeki web adresleri, hiçbir ayar yapmadan tıklanabilir bağlantılara dönüşür. `https://` veya `www.` ile başlayan adreslerin yanı sıra `teable.ai` gibi yaygın alan adları da tanınır. Bir değerde birden fazla adres varsa her biri ayrı bir bağlantı olur. Kaydı genişlettiğinizde, giriş alanının yanındaki bağlantı düğmesi hücredeki tüm adresleri listeler.

`readme.md` veya `v1.2.3` gibi dosya adları ve sürüm numaraları adres sanılmaz.

## Görüntüleme Modu

Tek Satır Metin üç görüntüleme modunu destekler:

| Görüntüleme modu | Uygun olduğu durumlar | Tıklama davranışı |
| --- | --- | --- |
| Varsayılan | Adlar, başlıklar, kimlikler ve diğer düz metinler | Adresler bağlantıya dönüşür ve tıklandığında açılır |
...
- **Müşteri iletişim bilgileri yönetimi**: Bir "Şirket Web Sitesi" alanı oluşturup adresi yazın; tıklandığında web sitesi açılır. Mobil kullanıcıların tek dokunuşla arayabilmesi için bir "Telefon" alanı oluşturun ve **Telefon** olarak ayarlayın.
...
- E-posta ve Telefon görüntüleme modlarıdır. Alan, Tek Satır Metin olarak kalır ve sıralanmaya, filtrelenmeye ve metin işlevleriyle işlenmeye devam edebilir.
