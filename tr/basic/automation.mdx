...
Bir otomasyon dış servisleri sizin kimlik bilgilerinizle çağırır: bir OAuth bağlantısı ya da kaydettiğiniz bir gizli anahtar. İş akışını düzenleme sekmesinde açın, test denetimlerinin yanındaki **Kimlik bilgileri ve yapılandırma**'ya tıklayın, bu otomasyona bir kimlik bilgisi verin ve ona bir **Değişken adı** verin.

Komut dosyası eylemleri bu adı `process.env.EXTERNAL_API_KEY` gibi bir ortam değişkeninden okur; diğer eylemlerde bir giriş alanının yanındaki **Gizli anahtar yerleştir** ile referans eklersiniz. Değerler hiçbir zaman düz metin olarak gösterilmez, test sonuçlarında da maskelenir.

Kimlik bilgilerinin kendisi **Ayarlar** → **Entegrasyonlar** bölümünde yönetilir; bkz. [Kimlik Bilgileri ve Entegrasyonlar](/tr/basic/credential).
...
| Öğe | Sınır |
|---|---|
| E-posta gönderim hızı | Proje başına saniyede 5 |
| Webhook alma hızı | Proje başına saniyede 50, iş akışı başına saniyede 2 |
...
<AccordionGroup>
  <Accordion title="Neden otomasyon çalıştırma kullanımı uyarısı aldım?">
    ...
    Bildirim, **Alan Ayarları** → **Faturalandırma** bölümünü açar. Kullanılabilir olduğunda en fazla çalıştırmaya katkıda bulunan iş akışını ve Projeyi gösterir. Kota hatırlatmaları plan sınırınıza dahil edilen çalıştırmalara dayanır; olağan dışı kullanım denetimleri **başarılı**, **başarısız** veya **beklemede** durumundaki son çalıştırmaları da değerlendirebilir.
