...
Paylaşılan Görünümlerinizi, Projelerinizi veya yapıtlarınızı parola doğrulamasıyla koruyun. Etkinleştirildiğinde alıcıların paylaşılan içeriğe erişmeden önce doğru parolayı girmesi gerekir.

Parola korumasını açtığınızda Teable otomatik olarak rastgele bir parola oluşturur ve bu parola hemen geçerli olur. Parolanın yanındaki **Bağlantıyı ve şifreyi kopyala**'ya tıklayarak bağlantıyı ve parolayı tek satırlık bir metin olarak kopyalayıp doğrudan ziyaretçilere gönderebilirsiniz. Parolayı değiştirmek için düzenle düğmesine tıklayıp yeni bir parola girin veya yenisini almak için **Rastgele şifre oluştur**'a tıklayın.

Parola, yalnızca ayarlandıktan hemen sonra paylaşım panelinde açık metin olarak gösterilir. Paylaşım ayarlarını yeniden açtığınızda parola gizlenir; tekrar kopyalamanız gerekirse yeni bir parola belirleyin.

## Hesap ve Oturum Açma

### Oturum Açılan Cihazlar

Sol alttaki avatarınıza tıklayıp **Ayarlar** → **Devices** yolunu izleyerek hesabınızın şu anda hangi cihazlarda oturum açık olduğunu ve son 30 günün **Login history** kayıtlarını görebilirsiniz. Her cihaz için tarayıcı veya mobil uygulama, işletim sistemi, oturum açma yöntemi, IP adresi ve son etkinlik zamanı gösterilir.

Tanımadığınız bir cihaz görürseniz o cihazın **Sign out** düğmesine tıklayın ve ardından parolanızı değiştirin. **Sign out all other devices**'a tıklayarak geçerli cihaz dışındaki tüm cihazlardan tek seferde çıkış yapabilirsiniz. Çıkış yaptırılan cihazların kullanılmaya devam etmesi için yeniden oturum açılması gerekir.

### Başarısız Oturum Açma Kilidi

Aynı e-posta adresiyle art arda 5 kez yanlış parola girilirse hesap 15 dakika boyunca kilitlenir; bu süre içinde yapılan hatalı denemelerin tümü reddedilir.
...
|--------|-------------|----------|
| **[Projeyi Dışa Aktarma](/tr/basic/base#export-base-backup-&-migration)** | Projenin tamamını `.tea` dosyası olarak indirin (yapı, veriler, otomasyonlar) | Tam yedekleme, taşıma |
| **[Projeyi Çoğaltma](/tr/basic/base#duplicate-a-base-to-another-space)** | Teable içinde Projenin bir kopyasını oluşturun | Hızlı anlık görüntü |
...
<Tip>
Projenin tamamını `.tea` dosyası olarak, tek tek Tabloları CSV dosyaları olarak dışa aktararak veya verilerinizi Teable API üzerinden alarak Projelerinizi elle yedekleyebilirsiniz.
</Tip>
