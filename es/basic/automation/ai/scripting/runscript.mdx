...
`POST /api/automation/runtime/ai` envía un prompt al modelo de IA de tu Proyecto. El Proyecto se obtiene del contexto de la Automatización, por lo que no se necesita ningún ID de Proyecto. El cuerpo acepta `prompt`, además de los parámetros opcionales `attachments`, `modelKey`, `temperature` y `outputType`; la respuesta es `{ "message": ... }`.
