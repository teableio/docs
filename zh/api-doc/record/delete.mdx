...
* **recordIds（必填）**[（如何获取）](/zh/api-doc/get-id#recordid)
  * 描述：要删除的记录 ID 数组
  * 类型：数组
...
<CodeGroup>
  ...
  ```ts TypeScript
  const response = await fetch('https://app.teable.cn/api/table/__tableId__/record?recordIds[]=rec123456&recordIds[]=rec789012', {
    method: 'DELETE',
    headers: {
