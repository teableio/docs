...
## Hesabınızı Silerken Alanlarınız

Sol alttaki avatarınız → **Ayarlar** → **Hesabım** yolunu açıp **Hesabı Sil**'e tıklayın. İletişim kutusu önce yalnızca sizin sahip olduğunuz Alanları listeler; her biri için bir karar gerekir. Diğer Alanlar yalnızca sizi iş arkadaşı olarak kaybeder, başka bir değişiklik olmaz.

- Varsayılan olarak Alan, tıpkı kendiniz sildiğinizdeki gibi hesapla birlikte çöp kutusuna gider.
- Alanda başka üyeler varsa Alanı onlardan birine devredebilirsiniz; devir, hesap silinmeden önce gerçekleşir.
- Etkin aboneliği olan bir Alan hesapla birlikte çöp kutusuna gidemez. Önce aboneliği iptal edin ya da Alanı başka bir üyeye devredin.

Bu Alanları hallettikten sonra, istendiği gibi `DELETE` yazarak onaylayın. Silinen bir hesap geri getirilemez.

## Benimle Paylaşılanlar

Birisi sizi projeye ortak çalışan olarak eklediğinde proje, Alan kenar çubuğundaki **Benimle paylaşılanlar** altında görünür.

Sizinle paylaşılan projeleri, kendi Alanlarınızdaki projelerden farklıdır:

- Erişim, paylaşan kişi tarafından denetlenir.
- Kullanabileceğiniz işlemler, size verilen proje iznine bağlıdır.
- Paylaşan kişi erişiminizi kaldırırsa proje artık görünmez.

Herkese açık proje paylaşımı için [Proje](/tr/basic/base) sayfasına bakın.
...
**IM'de Sohbet** sayfasında Alan yöneticileri bot oluşturabilir ve botun modelini, erişebildiği Projeleri, Becerilerini, dosyalarını ve IM entegrasyonunu yapılandırabilir.
