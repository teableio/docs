...
- Filtra por **Espacio**, **Proyecto**, **Causa** o **Resultado**; busca una tarea, Proyecto, Espacio o error; o establece un tiempo mínimo de procesamiento para localizar las tareas lentas.
...
| **Cronología** | Tres ejes en paralelo: **Extremo a extremo** va del cambio de origen hasta que convergen las escrituras calculadas, **Esta tarea** abarca del claim al commit, y **Job de despertar** es el despertar que la ejecutó. El eje de la tarea se divide en espera en cola, claim / setup y cálculo. El job de despertar también drena otro trabajo vencido de la misma base, así que normalmente dura más que el extremo a extremo. |
| **Cadena de tareas** | Todas las tareas de la cascada, anidadas bajo la tarea que las añadió a la cola, para que puedas ver el origen de cada rama. Los lotes parciales derivados de una misma etapa forman una cola serie y se mantienen al mismo nivel en lugar de anidarse más. Cada fila muestra su estado, los pasos del plan, el **Destino de escritura** (la Tabla y los Campos que actualiza la tarea), la **Complejidad estimada**, la profundidad de la cascada, la hora de entrada en la cola y la duración. |
...
**Mantenimiento de anomalías** agrupa las anomalías por problema, una por cada Proyecto, Tabla de origen y error, para facilitar la detección de una única causa raíz. Revisa el motivo del fallo y corrige el problema subyacente antes de recuperar nada. Expande un grupo para consultar los detalles del error y las tareas que abarca; esas filas sirven para examinarlas, pero la recuperación siempre se ejecuta sobre el grupo completo.
...
Un grupo marcado como **Proyecto eliminado** pertenece a un Proyecto que ya no existe. Su recuperación volvería a fallar de inmediato, por lo que **Descartar grupo** es la única acción disponible.
...
Las pausas siempre caducan por sí solas y su duración máxima es de dos horas. Si el incidente dura más, haz clic en **Ampliar** en la fila de la pausa, elige cuánto tiempo más quieres mantenerla y confirma. La nueva reanudación automática se calcula desde el momento en que confirmas, de nuevo con un máximo de dos horas, así que el Espacio no se libera entretanto.
...
| **Límites de reclamación por Proyecto** | Cuántas tareas calculadas se ejecutan al mismo tiempo para un Proyecto (**Por Proyecto**) y para una Tabla de origen dentro de ella (**Por Tabla inicial**). Ambos valores son 2 de forma predeterminada y admiten valores del 1 al 16. |

Aumenta los límites de reclamación cuando el cuello de botella sea una única Proyecto con mucha actividad, y la simultaneidad por proceso cuando haya muchas Proyectos en cola a la vez.
