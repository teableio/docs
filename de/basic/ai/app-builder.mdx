...
Der kopierte Inhalt entspricht dem, was die App in der eigenen Sandbox von Teable erhält: die für die App freigegebenen Zugangsdaten sowie die Variablen, die die Plattform für den API-Endpunkt, das Zugriffstoken und den KI-Zugriff der App einfügt.
...
Um Zugangsdaten hinzuzufügen oder zu ersetzen, verwenden Sie **Zugangsdaten & Konfiguration** in Teable statt der lokalen Datei. Eine in GitHub committete `.env` erstellt oder aktualisiert keine Zugangsdaten. Kopieren Sie die Variablen nach einer Änderung erneut, um Ihre lokale `.env` zu aktualisieren.
