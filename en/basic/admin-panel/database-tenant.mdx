...
<AccordionGroup>
  ...
  <Accordion title="Does a database tenant connect an existing business database to Teable?">
    No. It changes where Teable stores and manages its own Space data. You provide a PostgreSQL database hosted elsewhere, and Teable creates and manages its own data there. Existing tables and records in that database are not connected to Teable or automatically imported as Projects.
  </Accordion>
</AccordionGroup>
