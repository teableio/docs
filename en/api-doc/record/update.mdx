...
<CodeGroup>
  ```bash CURL
  curl -X PATCH 'https://app.teable.io/table/__tableId__/record/__recordId__' \
  ...
  ```ts TypeScript
  const response = await fetch('https://app.teable.io/table/__tableId__/record/__recordId__', {
    method: 'PATCH',
    headers: {
  ...
  response = requests.patch(
      'https://app.teable.io/table/__tableId__/record/__recordId__',
      headers={
          'Authorization': 'Bearer __token__',
          'Content-Type': 'application/json'
      },
