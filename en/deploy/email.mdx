...
We recommend configuring mail service in **Admin Setting (Admin Panel → Settings → Email Configuration)** first. The effective priority is:
...
2. **Admin Setting → Email Configuration → Automation email**: Used by automation emails when the Send Email action does not configure its own mail server.
3. **Admin Setting → Email Configuration → Notify email**: Used for user verification, password reset, invitations, system notifications, and similar emails. It is also used as the fallback for automation emails if Automation email is not configured.
4. **`BACKEND_MAIL_*` environment variables**: Deployment-level default mail configuration. Used only as a fallback when the corresponding mailbox is not configured in Admin Setting.
...
<Note>
We recommend configuring Notify email and Automation email in **Admin Setting (Admin Panel → Settings → Email Configuration)** first. Environment variables are mainly used as deployment-level defaults and fallback configuration.
</Note>
