...
## I tuoi Spazi quando elimini l'account

Apri il tuo avatar in basso a sinistra → **Impostazioni** → **Il mio account**, poi fai clic su **Elimina account**. La finestra elenca per prima cosa gli Spazi di cui sei l'unico proprietario: ognuno richiede una decisione. Tutti gli altri Spazi perdono solo te come collaboratore e per il resto restano invariati.

- Per impostazione predefinita lo Spazio finisce nel cestino insieme all'account, esattamente come se l'avessi eliminato tu.
- Se lo Spazio ha altri membri, puoi affidarlo a uno di loro: il passaggio avviene prima dell'eliminazione dell'account.
- Uno Spazio con un abbonamento attivo non può finire nel cestino con l'account: annulla prima l'abbonamento oppure affida lo Spazio a un altro membro.

Sistemati questi Spazi, digita `DELETE` come richiesto per confermare. Un account eliminato non è recuperabile.

## Condivisi con me

Quando qualcuno condivide con te un Progetto come collaboratore, il Progetto viene visualizzato sotto **Condivisi con me** nella barra laterale dello Spazio.

I Progetti condivisi con te sono diversi dai Progetti presenti nei tuoi Spazi:

- L'accesso è controllato da chi condivide.
- Le azioni disponibili dipendono dall'autorizzazione concessa per il Progetto.
- Se chi condivide rimuove il tuo accesso, il Progetto non viene più visualizzato.

Per la condivisione pubblica dei Progetti, consulta [Progetto](/it/basic/base).
...
Nella pagina **Chat in IM**, gli amministratori dello Spazio possono creare bot e configurarne modello, Progetti accessibili, skill, file e integrazione IM.
