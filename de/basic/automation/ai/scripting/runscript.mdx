...
`POST /api/automation/runtime/ai` sendet einen Prompt an das KI-Modell Ihrer Projekt. Das Projekt stammt aus dem Kontext der Automatisierung, daher ist keine Projekt-ID erforderlich. Der Text erwartet `prompt` sowie optional `attachments`, `modelKey`, `temperature` und `outputType`; die Antwort lautet `{ "message": ... }`.
