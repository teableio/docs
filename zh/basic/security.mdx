...
使用密码保护您分享的视图、项目或 Artifact。启用后，接收者必须输入正确的密码才能访问分享的内容。

开启密码保护时，Teable 会自动生成一个随机密码并立即生效。点击密码旁的 **复制链接和密码**，可以把链接和密码作为一行文字一起复制，直接发给访问者。需要更换时点击编辑按钮，输入新密码，或点击 **随机生成** 换一个。

密码只在刚设置后的分享面板中显示明文。重新打开分享设置后，密码会被隐藏；如需再次复制，请重新设置一个密码。

## 账户与登录

### 登录设备

打开左下角头像 → **设置** → **登录设备**，可以查看账号当前在哪些设备上登录，以及最近 30 天的 **登录日志**。每台设备会显示浏览器或移动端 App、操作系统、登录方式、IP 地址和最近活跃时间。

发现不认识的设备时，点击该设备的 **退出登录**，然后修改密码。点击 **退出其他所有设备** 可以一次退出除当前设备外的所有设备。被退出的设备需要重新登录才能继续使用。

### 登录失败锁定

同一邮箱连续输错密码 5 次后，账号会被锁定 15 分钟，期间继续输错的尝试都会被拒绝。
...
| 方式 | 说明 | 适用场景 |
|------|------|----------|
| **[项目导出](/zh/basic/base#导出数据库（备份与迁移）)** | 将整个项目下载为 `.tea` 文件（包含结构、数据、自动化） | 完整备份、迁移 |
| **[项目复制](/zh/basic/base#duplicate-a-base-to-another-space)** | 在 Teable 内创建项目副本 | 快速快照 |
| **[CSV 导出](/zh/basic/table/export)** | 导出单个表格数据 | 数据迁移 |
| **[API 导出](/zh/api-doc/record/get)** | 通过 REST API 程序化导出记录 | 自动化备份 |

<Tip>
  您可以通过导出整个项目为 `.tea` 文件、导出单个表格为 CSV 文件，或通过 Teable API 获取数据来手动备份您的项目。
</Tip>
