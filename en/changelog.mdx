...
* **Added Teable Chat App context**: Teable Chat uses authorized, @mentioned Apps’ code, structure, and configuration while excluding sensitive runtime settings.

* **Added short links for sharing and templates**: Use simpler links for easier sharing and access.

## Fixes & Improvements

* **Improved Recycle Bin stability**: Fixed errors when opening bulk-deleted records and made large-batch loading and restoration more reliable.

* **Improved cross-Base calculated field updates**: Prevented updates from blocking or failing, ensuring timely and accurate data.

* **Improved view-switching performance**: Reduced duplicate loading and unnecessary refreshes for smoother transitions.

* **Improved field calculation status display**: Clarified progress and status, simplified the activity panel, and added translations.

* **Improved bulk-deletion previews**: The Recycle Bin now shows sample deleted records and the remaining count.

* **Improved large table performance**: Accelerated loading for tables with many columns, enabling smoother browsing and operations.

- **Fixed manual sorting issues**: Record order now remains consistent after real-time updates or page reloads.

- **Improved self-link record picker**: Displays only visible fields from the current link and returns complete records.

- **Improved table description layout**: Reduced maximum width to prevent obscuring view tabs and improve header usability.

- **Unified link field visibility rules**: API updates now retain linked tables’ primary fields in link fields and sharing settings.

- **Improved attachment and data import stability**: Increased trusted-source request reliability, reducing failures in runtime-configured environments.

- **Fixed shared Base copying**: Copies now succeed despite out-of-scope plugins, including shared tables and panels while skipping invalid content.
