...
<AccordionGroup>
  ...
  <Accordion title="Le Chat IA peut-il fonctionner dans un autre projet ?">
    Oui. L’IA n’est pas limitée au Projet à laquelle appartient le chat ; elle peut rechercher les autres Projets auxquelles vous avez accès et y travailler. Nommez le Projet dans votre demande, par exemple « comparer cette table avec la table des commandes dans le Projet Ventes ». `@` répertorie uniquement les nœuds du Projet actuel, et tout ce que vous ne dirigez pas ailleurs se produit toujours dans le Projet propre au chat.
  </Accordion>
