...
To add or replace a credential, use **Credentials & config** in Teable rather than the local file. A `.env` committed to GitHub does not create or update a credential. After a credential changes, copy the variables again to refresh your local `.env`.
