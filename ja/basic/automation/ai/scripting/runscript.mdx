...
`POST /api/automation/runtime/ai` は、プロジェクトのAIモデルへプロンプトを送信します。プロジェクトはオートメーションのコンテキストから取得されるため、プロジェクトIDは不要です。本文には `prompt` のほか、任意の `attachments`、`modelKey`、`temperature`、`outputType` を指定でき、レスポンスは `{ "message": ... }` です。
