...
1. 进入 Teable 账户的[设置 > OAuth 应用](https://app.teable.ai/setting/oauth-app)页面。
...
| **项目** | `base\|create`, `base\|delete`, `base\|read`, `base\|read_all`, `base\|update`, `base\|table_import`, `base\|table_export`, `base\|query_data`, `base\|authority_matrix_config` |
| **表格** | `table\|create`, `table\|delete`, `table\|export`, `table\|import`, `table\|read`, `table\|update`, `table\|trash_read`, `table\|trash_update`, `table\|trash_reset`, `table\|archive_read`, `table\|archive_manage` |
| **视图** | `view\|create`, `view\|delete`, `view\|read`, `view\|update` |
| **字段** | `field\|create`, `field\|delete`, `field\|read`, `field\|update` |
| **记录** | `record\|comment`, `record\|create`, `record\|delete`, `record\|read`, `record\|update`, `record\|archive` |
| **自动化** | `automation\|create`, `automation\|delete`, `automation\|read`, `automation\|update` |
| **例行任务** | `routine\|create`, `routine\|delete`, `routine\|read`, `routine\|update` |
| **用户** | `user\|email_read`, `user\|integrations`, `user\|spaces_read`, `user\|self_hosted_licenses_read`, `user\|notifications_send` |

以下用户权限范围用于读取账户信息或触达用户：

| 权限范围 | 用途 |
|------|------|
| `user\|spaces_read` | 在用户信息接口 `GET /api/auth/user` 的返回中增加 `spaces`：用户占用席位的空间，以及每个空间的订阅等级、席位数和是否试用。仅 Teable Cloud 返回 |
| `user\|self_hosted_licenses_read` | 在用户信息接口的返回中增加 `selfHostedLicenses`：用户购买且仍有效的私有化部署许可证，以及订阅等级、席位数和是否试用。仅 Teable Cloud 返回 |
| `user\|notifications_send` | 向授权用户发送站内通知，见 [向用户发送通知](#向用户发送通知) |
...
如果用户之前已授权过您的应用（默认 7 天内有效），且本次请求的权限范围都已被批准过，将直接重定向而不再显示授权页面。请求了新的权限范围时，用户需要重新确认；确认后，之前批准的范围仍然保留。
...
设备授权流程（[RFC 8628](https://datatracker.ietf.org/doc/html/rfc8628)）适用于收不到浏览器回调的客户端：通过 SSH、容器或云端 IDE 运行的 CLI。客户端显示一个网址和一个短授权码，用户在任意浏览器中确认，不需要把任何内容粘回终端。

Teable 遵循 RFC 8628，大多数 OAuth 客户端库不用改代码就能跑通这个流程。下面只列 Teable 这一侧的约定。

<Warning>设备授权流程默认关闭，使用前需要在 OAuth 应用设置中开启 **启用设备授权流程**。任何知道 client ID 的人都能以该应用的名义发起此流程，请仅在应用确实需要时开启。关闭后，已经在等待确认的请求也会一并中止。</Warning>
...
两个码的有效期都是 15 分钟（`BACKEND_OAUTH_DEVICE_CODE_EXPIRE_IN`），`interval` 是两次轮询之间至少要等待的秒数。

同时打印 `verification_uri` 和 `user_code`。用户在该页面登录后输入授权码，确认应用名称、主页和申请的权限范围，然后同意或拒绝。页面会提醒用户不要确认并非自己发起的授权码。每个授权码只能使用一次。

<Note>Teable 不返回 `verification_uri_complete`，客户端也不要自己拼一个。授权码一旦被确认，就等于把确认者的 Teable 账号登录到了那台设备上，而带着授权码的链接正是设备码钓鱼所依赖的形式。</Note>
...
| 错误 | 客户端应如何处理 |
|------|------------------|
| `authorization_pending` | 还没有人确认，按 `interval` 继续轮询 |
| `slow_down` | 轮询过于频繁，下一次轮询前等待更久 |
| `access_denied` | 用户拒绝了请求，停止轮询 |
| `expired_token` | 授权码已过期或已被使用，需要重新开始 |
...
该接口返回当前用户有权限访问的所有项目，可从中获取 `baseId` 用于后续 API 调用。

## 向用户发送通知

获得 `user|notifications_send` 权限范围后，应用可以用访问令牌向该用户发送站内通知。通知会显示在用户的通知中心，并注明来自哪个应用。

```bash
curl -X POST https://app.teable.ai/api/notifications \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"externalId": "order-1024-shipped", "text": "订单 1024 已发货", "url": "https://yourapp.com/orders/1024"}'
```

| 参数 | 必填 | 描述 |
|------|------|------|
| `externalId` | 是 | 应用自己的通知 ID，1-128 位字母、数字或 `_` `.` `:` `-`。同一 ID 重复发送给同一用户不会产生新通知 |
| `text` | 是 | 通知正文，纯文本，最多 500 个字符。Teable 不会翻译，请使用用户的语言 |
| `url` | 否 | 点击通知后打开的地址，必须是 https，且主机与应用主页或某个回调 URL 相同 |

响应中的 `status` 为 `created` 表示已送达，`duplicate` 表示该 `externalId` 已发送过，`muted` 表示用户关闭了该应用的通知。每个应用对同一用户每分钟最多发送 30 条，全部用户合计每分钟最多 600 条，超出时返回 429 并带有 `Retry-After`。
...
用户也可以通过[已授权应用](https://app.teable.ai/setting/authorized-apps)设置页面撤销。
...
| 设备授权码 | 15 分钟 | `BACKEND_OAUTH_DEVICE_CODE_EXPIRE_IN` |
...
| `unauthorized_client` | 该 OAuth 应用没有开启设备授权流程 |
| `too_many_requests` | 超过速率限制。Token 请求默认每 15 分钟 30 次，设备授权码请求默认每个 IP 每 15 分钟 30 次 |

## 最佳实践

1. **选择合适的模式**：有后端服务器的 Web 应用使用客户端密钥模式，原生应用/CLI/SPA 使用 PKCE 模式，收不到浏览器回调的客户端使用设备授权流程
