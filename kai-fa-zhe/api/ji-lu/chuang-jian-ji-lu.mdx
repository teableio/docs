...
* **records（必填）**
  ...
  * 说明：每个记录都是一个包含 `fields` 对象的项。`fields` 对象包含字段名和对应的值，每一种字段值结构都不一样，详情请[查看记录字段值类型](/kai-fa-zhe/api/ji-lu/ji-lu-zi-duan-zhi-lei-xing)。
...
* **order（可选）**
  ...
  * 包含以下属性：
    ...
    * viewId
      ...
      * 描述：视图的 ID [（如何获取）](/kai-fa-zhe/api/huo-qu-id#viewid)
    ...
    * anchorId
      ...
      * 描述：锚点记录的 ID[（如何获取）](/kai-fa-zhe/api/huo-qu-id#recordid)
...
[上一页获取记录](/kai-fa-zhe/api/ji-lu/huo-qu-ji-lu)[下一页更新记录](/kai-fa-zhe/api/ji-lu/geng-xin-ji-lu)
