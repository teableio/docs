...
| Servidor de Correo Personalizado | No | Envía esta acción a través de una cuenta SMTP concreta sin afectar a las demás automatizaciones |

\* Se requiere al menos uno de los Campos **Para** o **CCO**.

**Límite de velocidad:** 5 correos electrónicos por segundo y por Proyecto.
...
### Dar a esta acción su propio SMTP

Para enviar solo esta acción a través de otra cuenta SMTP, haz clic en **Agregar Configuración** bajo **Servidor de Correo Personalizado** y completa la dirección del servidor, el puerto, el usuario y los datos del remitente.

La contraseña ya no se escribe aquí: haz clic en **Selecciona un secreto** para elegir uno de los secretos concedidos a esta automatización, o en **Añadir secreto** para crear uno (nombre de variable predeterminado `SMTP_PASSWORD`; una segunda cuenta SMTP en el mismo flujo se numera como `SMTP_PASSWORD_2`). La acción guarda una referencia, así que el valor en claro no entra en la configuración del flujo y el historial de ejecuciones y las exportaciones lo muestran enmascarado.

**Restablecer** borra esta configuración y la acción vuelve al servidor de correo predeterminado.

<Info>Este cuadro de diálogo no envía correos de prueba. Cuando termines de configurarlo, ejecuta el flujo una vez para comprobar el envío.</Info>
