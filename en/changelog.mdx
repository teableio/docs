...
# History and stability optimizations
...
* **Improved history stability for large tables**: Automatically archives, cleans, and compresses history to reduce performance pressure.

* **Improved record history pagination and loading**: Fixed pagination and timeout issues for more stable history viewing.
...
* **Fixed Date field display issue**: Date-only fields no longer include time in formulas or copied results.

* **Fixed date filtering issue**: Date conditions like “later than or equal to” no longer break automation scripts.

* **Fixed attachment preview issue**: Onboarding attachments now preview more reliably after entering the app.

* **Improved AI streaming output**: AI responses stream more smoothly, reducing frontend waiting and lag.

* **Improved AI content saving**: Generated content is better preserved if streaming is interrupted or fails.

* **Fixed linked record display issue**: Linked records no longer temporarily show “Untitled” after updating a Number field.

* **Improved required field setting prompts**: Failed saves now show clearer errors for missing required configuration.

* **Added localized error messages**: Required field validation messages now follow the user’s language.
...
* **Improved billing stability**: Charges and refunds are more reliable, reducing delays, duplicate charges, and refund errors.

* **Fixed date field display issue**: Date-only fields no longer include time in formulas or copied results.

* **Fixed date filtering issue**: Automation scripts no longer fail when using date conditions like “equal to or later than”.

* **Fixed onboarding attachment previews**: Onboarding uploads now preview more reliably in the app, reducing expired-link failures.

* **Improved AI app creation**: AI structures user requirements into clearer prompts, helping App Builder generate apps more accurately.

* **Improved Agent Computer startup speed**: Reduced unnecessary startup checks so AI Agent returns the first response faster.

* **Improved connector menu display**: The “Connect everything” entry is clearer and less likely to be confused.

* **Improved Agent file reading**: Large files are read more reliably, reducing failures caused by excessive content.

* **Fixed workflow record retrieval issue**: Missing or invalid `create time` no longer causes record retrieval failures.

* **Unified workflow node time format**: Node timestamps now use ISO format for more stable validation and service handling.

* **Fixed required single-select field creation**: Existing records get default values before required constraints are applied.

* **Fixed Enterprise License validation**: Existing self-hosted License Keys remain valid after upgrades, avoiding false “license is invalid” errors.

* **Fixed website onboarding duplicate attachments**: Initial website attachments no longer appear twice in auto-submitted prompts.

* **Fixed false App Builder “unpublished changes” warning**: Published apps no longer require republishing when configuration is unchanged.

* **Improved environment variable change detection**: Encryption-only or timestamp changes are no longer treated as configuration changes.

* **Fixed Airtable import failures from duplicate select options**: Imports handle trimmed duplicate options in single-select and multi-select fields more reliably.

* **Fixed duplicate message sending**: Resolved cases where Teable AI could send duplicate messages.

* **Fixed required single-select record creation**: Records can now be created when the required field has a default value.

* **Improved recovery entry for error tables**: Recovery actions remain available when a table enters an error state.

* **Strengthened comment permission control**: Comment details, reactions, updates, deletions, and references are restricted to their table and record.

* **Improved Link field data repair**: Missing linked records are handled more reliably during cleanup and repair.
...
* **Improved archiving stability**: Large history tables archive more reliably, preventing oversized records from failing the entire task.

* **Improved large record processing performance**: Archive reads and merges are more stable, with lower memory usage under load.
