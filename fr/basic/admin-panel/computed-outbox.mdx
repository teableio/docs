...
- Filtrez par **Espace**, **Projet**, **Cause** ou **Résultat**, recherchez une tâche, un Projet, un Espace ou une erreur, ou définissez un temps de traitement minimal pour faire apparaître les tâches lentes.
...
| **Chronologie** | Trois axes côte à côte : **Bout-en-bout** va de la modification source à la convergence des écritures calculées, **Cette tâche** couvre la prise en charge jusqu’au commit, et **Job de réveil** correspond au réveil qui l’a exécutée. L’axe de la tâche est découpé en attente file, prise / setup et calcul. Le job de réveil vide aussi le reste dû sur la même base, il dépasse donc normalement le bout-en-bout. |
| **Chaîne de tâches** | Toutes les tâches de la cascade, imbriquées sous la tâche qui les a mises en file d’attente, afin de voir l’origine de chaque branche. Les lots partiels issus d’une même étape forment une file série et restent à plat sur un seul niveau au lieu d’être imbriqués plus profondément. Chaque ligne indique son état, les étapes du plan, la **Cible d’écriture** (la table et les champs que cette tâche met à jour), la **Complexité estimée**, la profondeur de cascade, l’heure de mise en file d’attente et la durée. |
...
Un groupe marqué **Projet supprimé** appartient à un Projet qui n’existe plus. Sa récupération échouerait immédiatement à nouveau ; **Ignorer le groupe** est donc la seule action proposée.
...
Une suspension expire toujours d’elle-même, et deux heures est la durée maximale que vous pouvez définir. Si l’incident dure plus longtemps, cliquez sur **Prolonger** dans la ligne de la suspension, choisissez la durée supplémentaire et confirmez. La nouvelle reprise automatique est calculée à partir de la confirmation, toujours dans la limite de deux heures, de sorte que l’Espace n’est pas libéré entre-temps.

La liste affiche chaque suspension active avec son créateur, sa raison et sa date de reprise automatique, ainsi que le nombre de tâches en attente retenues par les suspensions actives, afin que vous puissiez évaluer l’arriéré qui sera libéré lors de la reprise. Pour lever une suspension plus tôt, cliquez sur **Reprendre** dans sa ligne et confirmez.
...
| **Plafonds de prise en charge par Projet** | Nombre de tâches calculées exécutées simultanément pour un Projet (**Par Projet**) et pour une table source au sein de celle-ci (**Par table initiale**). Les deux valeurs sont définies sur 2 par défaut et acceptent des valeurs de 1 à 16. |

Augmentez les plafonds de prise en charge lorsqu’un Projet très sollicitée constitue le goulot d’étranglement, et la concurrence par processus lorsque de nombreuses Projets sont mises en file d’attente simultanément.
