...
- **Modelo**: Usa el menú de modelos para elegir el nivel de modelo de la conversación. El menú lista **Ultra**, **Smart**, **Standard** y **Lite**, del más capaz al menos capaz, e indica el modelo que hay detrás de cada uno; tu administrador puede cambiar ese modelo en cualquier momento u ofrecer solo algunos niveles, y tú no tienes que volver a elegir. Usa un nivel más bajo para consultas sencillas, limpieza o reescritura, y uno más alto para planificación compleja, análisis entre Tablas y creación de aplicaciones. En Cloud, el menú indica además en cada nivel el multiplicador de potencia respecto al nivel predeterminado (por ejemplo, **2.5×**); no aparece cuando el nivel consume aproximadamente lo mismo que el predeterminado o no consume potencia de la plataforma. Cambiar de nivel durante una conversación pide una confirmación; la conversación se mantiene y el nuevo modelo se hace cargo a partir de tu siguiente mensaje.
...
- **Mis secretos**: Abre **+** → **Más** → **Mis secretos** para guardar claves de API u otras credenciales en **Ajustes** → **Integraciones**. Esos valores son tuyos, el Chat de IA puede leerlos como variables de entorno durante la ejecución y puedes concederlos a aplicaciones y automatizaciones concretas. Consulta [Credenciales e integraciones](/es/basic/credential).
- **Tarjetas de autorización**: Cuando la IA necesita una cuenta externa o un secreto, fija una tarjeta en la conversación y espera antes de seguir con el turno. Si es Cuppy quien necesita la cuenta para hacer la tarea, pulsa **Conectar** para iniciar un flujo de OAuth; si la credencial la necesita una aplicación o automatización al ejecutarse, elige **Conectar mi cuenta** o concede una credencial que ya tengas. Con **Omitir** no la facilitas; la IA continúa con las partes que puede completar sin ella. Consulta [Credenciales e integraciones](/es/basic/credential).
- **Scraper**: Abre el menú **+**, elige **Scraper** y selecciona una plataforma y un tipo de dato. Hay dos clases de entradas: unas extraen a partir de un enlace que pegas, como **perfil personal** en LinkedIn o **reseñas del producto** en Amazon; otras buscan por palabra clave, hashtag o nombre, como **búsqueda de vídeos** en TikTok o **publicaciones de un subreddit** en Reddit. Teable rellena la instrucción correspondiente en la entrada del chat y deja al final la parte que debes completar. **Cualquier página web** acepta directamente un enlace pegado.
- **Habilidades**: Abre el menú **+** y elige **Habilidades** para importar, habilitar, deshabilitar o probar habilidades en el chat actual. Cuando importes una, elige quién puede usarla: **Personal** la reserva para ti, **Proyecto** la ofrece a los colaboradores de este Proyecto y **Espacio** la ofrece a todos los usuarios de este Espacio. Escribe `/` en el cuadro de entrada para elegir una habilidad habilitada. Para conectar o migrar un sistema externo, consulta [Conecta y migra todo](/es/basic/ai/connect-everything).
...
- **Cola de mensajes**: Enviar otro mensaje mientras la IA está trabajando no la interrumpe. El mensaje espera en una cola sobre el cuadro de entrada y se envía cuando termina la ejecución actual. Los elementos en cola conservan las mismas etiquetas de Tabla, Vista, archivo adjunto y selección que la entrada del chat; **Eliminar** descarta uno, **...** → **Editar mensaje** lo devuelve al cuadro de entrada y los elementos que solo contienen texto también ofrecen **Redirigir** para entregar el texto al turno en ejecución en lugar de esperar. Si el turno está detenido en una tarjeta que espera tu respuesta (una tarjeta de autorización, una pregunta de la IA), un mensaje de solo texto no entra en la cola: la tarjeta se cierra y queda marcada como **Omitido**, y tu mensaje pasa directamente a ese turno como su siguiente instrucción, así que no tienes que resolver antes la tarjeta. Los mensajes con archivos adjuntos siguen yendo a la cola.

## Gestionar los chats

Haz clic en **Historial**, arriba a la derecha del panel de chat, para ver tus chats del proyecto actual y buscarlos por nombre.

El punto junto al nombre de un chat señala cuáles te necesitan. **Esperando tu respuesta** indica que el turno se ha detenido en un punto que te requiere: una pregunta del agente, una autorización, una tabla que elegir o una solicitud de credencial. No continuará hasta que respondas. **La respuesta falló** significa que la última respuesta no terminó correctamente; ábrela y vuelve a preguntar. **Generando…** y el punto de no leído solo informan del progreso y de las respuestas nuevas, y no requieren nada de ti.

El menú **...** de un chat ofrece **Fijar**, **Renombrar**, **Archivar** y eliminar. Eliminar borra también todos sus mensajes y no se puede deshacer.

El historial muestra los 100 chats más recientes. Fija los que uses a menudo: un chat fijado encabeza la lista y queda fuera de ese límite, así que no lo desplazan los chats nuevos. También puedes arrastrar un chat para cambiarlo de sitio.

Los chats del Constructor de aplicaciones llevan el nombre de su aplicación y no se pueden archivar.

### Chats archivados

Archivar saca un chat del historial sin borrar nada. Abre tu avatar de la esquina inferior izquierda → **Configuración** → **Chats archivados** para ver los chats que archivaste en distintos proyectos. **Restaurar** devuelve uno al historial de su proyecto, y aquí también puedes eliminarlo cuando tengas claro que ya no lo necesitas.
...
Debajo de cada respuesta, **Cambios de este turno** enumera lo que cambió el turno, recurso por recurso: campos, vistas y registros añadidos, actualizados o eliminados, y aplicaciones y automatizaciones publicadas o activadas y desactivadas. Haz clic en una entrada para abrir ese recurso y comprobarlo.
...
Pide a la IA que produzca imágenes directamente: conceptos de producto, ilustraciones para artículos, bocetos de carteles:

- Dibuja una escena que muestre este producto en uso.
- Genera una imagen de portada a partir de la descripción de este registro.
- Toma esta imagen y dame tres más con colores de invierno.

Añade una imagen existente al chat para usarla como referencia, siempre que el modelo elegido admita imagen a imagen.

En el chat aparece una tarjeta de generación: indica qué modelo está trabajando y cuántas imágenes quedan, y luego muestra el resultado como una galería que puedes ampliar con un clic. Las imágenes se guardan en el sandbox del chat y se descargan desde **Administrar archivos**. Si detienes la ejecución, se cancela todo lo que quede pendiente.

<Info>El Chat de IA, el [App Builder](/es/basic/ai/app-builder) y las [rutinas](/es/basic/ai/routine) pueden generar imágenes; hablar con Cuppy desde una app de mensajería, no.</Info>
...
La memoria almacena la información que pides explícitamente a Cuppy que recuerde. Su ámbito se define por usuario y Proyecto: cada usuario de cada Proyecto tiene un contexto de memoria independiente, y la memoria guardada en otro Proyecto no aparece automáticamente en el Proyecto actual.

- Guardar memoria en el Proyecto actual
...
- Reutilizar la memoria de otro Proyecto

Pide a Cuppy que lea otro Proyecto y guarde las partes pertinentes en el Proyecto actual:

```text
Cuppy, lee la memoria del Proyecto con ID bsexxxxxxxxxxxx y guarda la memoria pertinente en este Proyecto actual.
```
...
<Steps>
  ...
  <Step title="Añadir una contraseña (opcional)">
    En los ajustes avanzados, activa **Protección con contraseña** y Teable generará una contraseña automáticamente. Los usuarios deben introducirla antes de que se cargue el artefacto. Haz clic en **Copiar enlace y contraseña** para copiar ambos a la vez.
  </Step>
...
- **Elige el nivel según la tarea**: Un nivel más bajo consume menos potencia en consultas sencillas, limpieza y reescritura; sube de nivel para planificación compleja, análisis entre Tablas y creación de aplicaciones.
...
<AccordionGroup>
  ...
  <Accordion title="¿Puede el Chat de IA trabajar en otro Proyecto?">
    Sí. La IA no está limitada al Proyecto al que pertenece el chat; puede buscar y trabajar en los demás Proyectos a los que tienes acceso. Indica el Proyecto en tu solicitud, por ejemplo: "compara esta Tabla con la Tabla de pedidos del Proyecto Ventas". `@` solo enumera los nodos del Proyecto actual, y cualquier acción para la que no indiques otro destino se ejecutará en el propio Proyecto del chat.
  </Accordion>
  ...
  <Accordion title="¿Cuántas imágenes produce una petición?">
    Cinco de forma predeterminada. Si necesitas más, pide a la IA que trabaje en varias tandas. El número de imágenes que un espacio puede generar a la vez también tiene tope, y lo que lo supere espera en la cola.
  </Accordion>

  <Accordion title="¿Qué modelo genera las imágenes?">
    Los modelos de imagen disponibles vienen de la configuración de IA del Proyecto: primero los modelos propios del espacio y después los que ofrece Teable. Si el Proyecto no tiene ningún modelo de imagen disponible, la IA no ofrece la generación de imágenes.
  </Accordion>

  <Accordion title="¿Cómo se cobran las imágenes generadas?">
    Una imagen consume créditos solo cuando queda guardada; las que fallan o se cancelan no cuestan nada. En el **Resumen de uso de créditos** de la página de facturación, estos cargos aparecen bajo el tipo **Generación de imágenes con IA**. Si el espacio tiene modelos propios (BYOK), quedarse sin créditos no impide generar con ellos.
  </Accordion>

  <Accordion title="¿Se comparte la memoria entre Proyectos?">
    No. El ámbito de la memoria se define por usuario y Proyecto. Si quieres reutilizar la memoria de otro Proyecto, pide a Cuppy que lea la memoria de ese Proyecto y guarde las partes pertinentes en el Proyecto actual.
  </Accordion>
  ...
  <Accordion title="¿Cómo uso las habilidades en el Chat de IA?">
    Abre el menú **+** y elige **Habilidades** para importar o administrar habilidades, o escribe `/` en el cuadro de entrada para elegir una habilidad habilitada. Al importarla, elige quién puede usarla: personal para tus propios chats, Proyecto cuando la necesiten los colaboradores del mismo Proyecto y Espacio cuando la necesiten todos los usuarios de ese Espacio. Los usuarios que pueden administrar el Proyecto pueden añadir habilidades de Proyecto, y los propietarios y creadores del Espacio pueden añadir habilidades de Espacio. Si el Chat de IA presenta un archivo `.skill`, usa **Instalar** en la tarjeta del archivo para añadirlo.
  </Accordion>

  <Accordion title="¿Qué habilidad se aplica cuando dos comparten el mismo nombre?">
    Solo se aplica una versión, y la versión compartida con el equipo tiene prioridad sobre una copia personal: las habilidades de Proyecto tienen prioridad sobre las de Espacio, y las de Espacio sobre las personales. En una conversación de una aplicación o bot, la habilidad añadida para esa aplicación o bot tiene prioridad. Para trasladar a todos a una versión nueva, actualiza la copia del Proyecto o el Espacio.
  </Accordion>
  ...
  <Accordion title="¿Dónde debo colocar las claves de API del Chat de IA?">
    Usa **+** → **Más** → **Mis secretos** en la entrada del chat. El Chat de IA puede usar esos valores durante la conversación sin incluir el texto sin formato en tu instrucción.
  </Accordion>
  <Accordion title="¿Qué hago si la plataforma que necesito no está en el menú de Scraper?">
    El menú reúne las plataformas habituales. Describe con tus palabras qué sitio y qué contenido quieres y la IA buscará la fuente de datos adecuada.
  </Accordion>
  <Accordion title="¿Cuántos registros devuelve una extracción?">
    En contenidos de lista como resultados de búsqueda, publicaciones y comentarios, cada enlace o palabra clave devuelve como máximo 10 registros de forma predeterminada. Indica una cantidad si necesitas más, hasta 50. Extraer una sola página devuelve ese único registro y no se ve afectado por este límite.
  </Accordion>
</AccordionGroup>
