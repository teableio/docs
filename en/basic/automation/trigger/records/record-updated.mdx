...
- If your action must update the same field, consider using the [When record matches conditions](/basic/automation/trigger/records/record-matches-conditions) trigger instead, which fires only on transitions.
...
- [When record matches conditions](/basic/automation/trigger/records/record-matches-conditions) — fires only on state transitions, not every edit
- [When record created](/basic/automation/trigger/records/record-created) — fires on new records, not updates
- [Update record action](/basic/automation/actions/records/update-record) — the action counterpart for modifying records in workflows
