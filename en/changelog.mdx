...
# App Building and Stability Improvements

## Feature Updates

* **Larger preview mode in App Builder**: Adds an expanded preview to view apps in a larger interface before publishing.
* **Clearer App Builder buttons**: Adds text labels to icon buttons for better readability and accessibility.
* **More reliable App Builder version management**: Improves saving, loading, rollback, export, copy, build, and deployment, especially for larger projects.
* **More accurate App Builder status messages**: Shows more realistic progress states, such as initializing, thinking, working, and committing.

## Fixes & Improvements

* **Fixed first record creation in empty tables**: Creating the first record in an empty table no longer triggers generic `ERROR` messages.
* **Improved bulk field deletion performance**: Deleting multiple fields in large tables is faster and has less impact.
* **Fixed field deletion issues in read-only views**: Improves stability and reduces errors when deleting fields.
* **Fixed misaligned row actions in personal views**: Paste, clear, and delete now target visible rows more accurately after sorting and reordering.
* **Improved linked record handling in CSV append imports**: Linked records, Lookup values, and backlinks update more reliably after import.
...
* **Faster Base copying**: Optimizes batching and linked field handling to reduce wait time for large Bases.
...
* **Fixed chat file downloads**: Files generated in conversations now download from the correct location.
* **Improved Agent Computer stability**: The runtime can recover on demand, reducing dependency and environment issues.
* **Improved Agent Computer isolation**: Reduces unexpected reuse of old state between runs for more stable, controllable results.
* **Improved WebFetch reliability in Agent Computer**: Reduces cases where accessible URLs fail to fetch due to precheck failures.
* **Fixed OAuth redirects in published apps**: Teable or Google login now returns to the app instead of `0.0.0.0:8080`.
