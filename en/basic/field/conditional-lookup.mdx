...
<Steps>
  ...
  <Step title="Remove duplicate values">
    Turn on **Remove duplicate values** and each value is shown once. Deduplication is the last step: Teable takes the records that pass the filter, the sorting, and the record limit, then removes duplicates among their values, so the result can hold fewer values than the number of records you allowed.
  </Step>
</Steps>
