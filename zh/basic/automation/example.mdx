...
<Note>此页面已弃用，且不再出现在文档目录中；内容仅保留给旧链接访问。请参考当前自动化目录下的示例页面。</Note>
...
- 参考：[`获取 ID`](/zh/api-doc/get-id) 与 [`访问令牌`](/zh/api-doc/token)
...
- [`自动化接口文档`](/zh/api-reference/automation/put-base-workflow-action)
