...
Le contenu copié correspond à ce que l’application reçoit dans le propre bac à sable de Teable : les identifiants accordés à l’application, ainsi que les variables que la plateforme injecte pour le point de terminaison API de l’application, le jeton d’accès et l’accès à l’IA.
...
Pour ajouter ou remplacer un identifiant, utilisez **Identifiants et configuration** dans Teable plutôt que le fichier local. Un fichier `.env` validé dans GitHub ne crée ni ne met à jour d’identifiant. Après une modification, copiez à nouveau les variables pour actualiser votre fichier `.env` local.
