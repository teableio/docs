...
`POST /api/automation/runtime/ai` invia un prompt al modello IA del Progetto. Il Progetto deriva dal contesto dell'Automazione, quindi non è necessario alcun ID Progetto. Il corpo accetta `prompt` e, facoltativamente, `attachments`, `modelKey`, `temperature` e `outputType`; la risposta è `{ "message": ... }`.
