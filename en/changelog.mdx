...
# Multi-app generation, computation, and performance optimization

## Feature Updates

* **Added Tab selection**: Use Tab to accept highlighted Skills or Slash Commands, with clear hints and unaffected focus or mobile navigation.

* **Added multi-app generation progress**: Chat now displays separate generation progress for each app created from one prompt.

* **Added per-chat batch downloads**: Displayed files are now organized by chat and available for batch download.

## Bug Fixes & Improvements

* **Improved calculation stability**: Large cells no longer block Formula, Lookup, or Rollup updates; Compute Activity preserves errors until successful refresh.

* **Safer record creation near table limits**: Valid writes are preserved while unsafe operations are rejected, preventing one failure from affecting others.

* **Fixed date-time backfill failures**: Backfilled date-time values no longer cause table schema updates to fail.

* **Faster record writes and calculations**: Reduced latency for inserts, Upserts, and cascading Lookup or Formula updates in highly connected tables.

* **Stronger Personal Access Token controls**: Scoped tokens now strictly enforce configured action and resource permissions, preventing unauthorized operations.

* **Improved GitHub sync**: Sync status is more accurate, temporary branches auto-delete after recovery, and the baseline commit message is clearer.

* **Improved computed field reliability**: Better handling of frequent updates and stale dependencies reduces failures and redundant retries.

* **Cleaner search results**: Private or functional pages, including Login, Settings, Developer, and Base, now appear less often.

* **Fixed issue card option display**: Highlighted options now match submitted selections, accurately showing the final choice.

* **Improved attachment and avatar loading**: Better caching stabilizes public images, private previews, and newly updated avatars.

* **Clearer CSV import errors**: Failures now show their original cause instead of remaining in progress or displaying unrelated errors.

* **Fixed table access and statistics**: Resolved tables failing to open and restored statistics and aggregation features.

* **Improved AI text and image automation errors**: More accurate feedback covers inputs, capabilities, attachments, responses, rate limits, and timeouts.

* **Stronger linked-record validation**: Writes protect required links from clearing or deletion, while title refreshes preserve required display values.

* **Fixed record copying failures**: Records with linked or Lookup fields using date-time comparisons can now be copied successfully.

- **Improved Lookup and computed field updates**: Large Lookup conditions and broad cascades no longer cause server errors during bulk edits.
...
<video autoplay loop muted playsinline controls preload="auto" className="w-full aspect-video" src="https://dxshyegpql0u3hra.public.blob.vercel-storage.com/b3b2d75b607cd2fd89f61262fe568ff6.mp4" />
