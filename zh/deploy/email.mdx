...
建议优先在 **Admin Setting（管理面板 → 设置 → 邮件配置）** 中配置邮件服务。不同来源的生效顺序如下：

1. **发送邮件动作内的自定义邮件服务器**：只作用于当前自动化的这一个发送邮件动作。
2. **Admin Setting → 邮件配置 → 自动化邮箱**：当发送邮件动作没有单独配置邮件服务器时，自动化邮件默认使用这里的配置。
3. **Admin Setting → 邮件配置 → 通知邮箱**：用于用户验证、重置密码、邀请、系统通知等邮件；如果未配置自动化邮箱，也会作为自动化邮件的回退配置。
4. **环境变量 `BACKEND_MAIL_*`**：部署级默认邮件配置。只有在 Admin Setting 中没有配置对应邮箱时，才作为兜底配置使用。
...
<Note>
推荐优先在 **Admin Setting（管理面板 → 设置 → 邮件配置）** 中配置通知邮箱和自动化邮箱。环境变量主要作为部署级默认配置和兜底配置。
</Note>
