...
| BACKEND_SIGNIN_VERIFICATION_EXPIRES_IN     | ログイン確認コードの有効期間。未設定の場合は BACKEND_EMAIL_CODE_EXPIRES_IN を引き継ぎます | 10m              | -    | 10m                                              |
| BACKEND_SIGNIN_VERIFICATION_MAX_ATTEMPTS   | ログインコード 1 件あたりに許容される誤入力回数。超えるとコードは無効になり、取り直しが必要です | 5                | -    | 5                                                |
| **セキュリティと認証** |  |  |  |  |
...
| BACKEND_MAIL_INVITE_SPACE_NAME_MAX_LENGTH  | 招待メールの件名に含めるスペース名またはプロジェクト名の最大文字数。空または0の場合、切り詰めません | - | - | 32 |
| **セッション／JWT設定** |  |  |  |  |
...
| TABLE_LIMIT_TABLES_PER_BASE_MAX            | 1プロジェクトあたりのテーブルの最大数                                                   | 1000            | -        | 1000                                              |
...
| V2_TABLE_QUERY_OPS_ENABLED                 | テーブルクエリ運用を有効にするかどうか。無効の場合、管理画面の**テーブルクエリ運用**にデータは表示されません。変更後は再起動が必要です        | false           | -        | true                                              |
| **分析と監視** |  |  |  |  |
...
| PUBLIC_DATABASE_PROXY                      | プロジェクトの「データベース接続」パネルで生成される認証情報に使用する、外部からアクセス可能なデータベースアドレス（host:port）。外部からの読み取り専用 SQL アクセスを有効にするには設定してください | -               | -        | db.example.com:42345                              |
...
| BIG_TRANSACTION_TIMEOUT                    | 大規模なプロジェクトのエクスポートなど、大きな内部トランザクションのタイムアウト（ミリ秒） | 600000          | -        | 1200000                                           |
