...
Open a user's action menu and select **Sessions** to see the devices where the user is currently signed in, along with their login history. Click **Sign out** on a single device, or click **Sign out all devices**, to force the user to sign in again. Unless the account is deactivated, the user can still sign back in.
