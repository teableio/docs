...
| **Проект** | `base\|create`, `base\|delete`, `base\|read`, `base\|read_all`, `base\|update`, `base\|table_import`, `base\|table_export`, `base\|query_data`, `base\|authority_matrix_config` |
| **Таблица** | `table\|create`, `table\|delete`, `table\|export`, `table\|import`, `table\|read`, `table\|update`, `table\|trash_read`, `table\|trash_update`, `table\|trash_reset`, `table\|archive_read`, `table\|archive_manage` |
| **Представление** | `view\|create`, `view\|delete`, `view\|read`, `view\|update` |
| **Поле** | `field\|create`, `field\|delete`, `field\|read`, `field\|update` |
| **Запись** | `record\|comment`, `record\|create`, `record\|delete`, `record\|read`, `record\|update`, `record\|archive` |
| **Автоматизация** | `automation\|create`, `automation\|delete`, `automation\|read`, `automation\|update` |
| **Рутина** | `routine\|create`, `routine\|delete`, `routine\|read`, `routine\|update` |
| **Пользователь** | `user\|email_read`, `user\|integrations`, `user\|spaces_read`, `user\|self_hosted_licenses_read`, `user\|notifications_send` |

Следующие пользовательские области действия позволяют читать данные учетной записи или связываться с пользователем:

| Область действия | Назначение |
|------|------|
| `user\|spaces_read` | Добавляет `spaces` в ответ конечной точки информации о пользователе `GET /api/auth/user`: пространства, в которых пользователь занимает место, а также тарифный план, количество мест и признак пробного периода для каждого из них. Возвращается только в Teable Cloud |
| `user\|self_hosted_licenses_read` | Добавляет `selfHostedLicenses` в ответ конечной точки информации о пользователе: действующие лицензии на самостоятельное развертывание, приобретенные пользователем, с тарифным планом, количеством мест и признаком пробного периода. Возвращается только в Teable Cloud |
| `user\|notifications_send` | Отправка уведомлений в приложении авторизованному пользователю. См. [Отправка уведомлений пользователям](#отправка-уведомлений-пользователям) |
...
Если пользователь ранее авторизовал ваше приложение (по умолчанию в течение 7 дней) и все запрошенные области действия уже были одобрены, он будет перенаправлен немедленно, без повторного показа страницы авторизации. Если запрошены новые области действия, пользователь должен подтвердить их заново; ранее одобренные области после этого сохраняются.
...
Как правило, первым шагом после получения токена является получение всех Проектов, доступных текущему пользователю:
...
Эта конечная точка возвращает все Проекты, к которым текущий пользователь имеет разрешение на доступ. Для последующих вызовов API можно использовать `baseId` из ответа.

## Отправка уведомлений пользователям

Получив область действия `user|notifications_send`, приложение может с помощью токена доступа отправлять этому пользователю уведомления в приложении. Уведомления появляются в центре уведомлений пользователя с указанием приложения-отправителя.

```bash
curl -X POST https://app.teable.ai/api/notifications \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"externalId": "order-1024-shipped", "text": "Заказ 1024 отправлен", "url": "https://yourapp.com/orders/1024"}'
```

| Параметр | Обязательный | Описание |
|------|------|------|
| `externalId` | Да | Собственный идентификатор уведомления в вашем приложении: 1-128 букв, цифр или символов `_` `.` `:` `-`. Повторная отправка того же идентификатора тому же пользователю не создает нового уведомления |
| `text` | Да | Текст уведомления, только обычный текст, не более 500 символов. Teable его не переводит, поэтому пишите на языке пользователя |
| `url` | Нет | Адрес, который открывается при нажатии на уведомление. Должен использовать https, а его хост должен совпадать с домашней страницей приложения или одним из URL обратного вызова |

Поле `status` в ответе принимает значение `created`, если уведомление доставлено, `duplicate`, если этот `externalId` уже отправлялся, и `muted`, если пользователь отключил уведомления от этого приложения. Каждое приложение может отправлять одному пользователю не более 30 уведомлений в минуту и не более 600 в минуту всем пользователям вместе. При превышении лимита возвращается 429 с заголовком `Retry-After`.
