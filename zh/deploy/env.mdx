---
title: "环境变量"
description: "这里列举了 Teable 所有可用的环境变量以及相关解释"
mode: "wide"
---
...
| SECRET_KEY                                  | 用于 JWT、会话和共享的根密钥，请生成强随机值                                | -                | 是    | `openssl rand -base64 32`                        |
| BACKEND_JWT_SECRET                          | 签发登录、分享和插件 JWT 的密钥，未设置时回退到 SECRET_KEY | SECRET_KEY | - | `openssl rand -base64 32` |
| BACKEND_SESSION_SECRET                      | 签发登录会话 Cookie 的密钥，未设置时回退到 SECRET_KEY | SECRET_KEY | - | `openssl rand -base64 32` |
| BACKEND_STORAGE_ENCRYPTION_KEY              | 附件访问令牌的加密密钥。BACKEND_STORAGE_PROVIDER 为 `local` 时必填 | - | 是 | 16 位随机字符串 |
| BACKEND_STORAGE_ENCRYPTION_IV               | 与 BACKEND_STORAGE_ENCRYPTION_KEY 配对的加密 IV | - | 是 | 16 位随机字符串 |
| BACKEND_ACCESS_TOKEN_ENCRYPTION_KEY         | 个人访问令牌的加密密钥 | - | 是 | 16 位随机字符串 |
| BACKEND_ACCESS_TOKEN_ENCRYPTION_IV          | 与 BACKEND_ACCESS_TOKEN_ENCRYPTION_KEY 配对的加密 IV | - | 是 | 16 位随机字符串 |
...
| BACKEND_STORAGE_S3_FORCE_PATH_STYLE      | 让浏览器访问 S3 时使用路径式 URL（`endpoint/bucket/key`）。当公网 endpoint 无法提供 bucket 子域名时启用。不能与 BACKEND_STORAGE_PRIVATE_BUCKET_ENDPOINT 同时使用 | false | - | true |
| BACKEND_STORAGE_S3_INTERNAL_FORCE_PATH_STYLE | 单独设置内网 S3 请求是否使用路径式访问。不设置时，如果公网和内网客户端共用 endpoint，则继承公网设置；使用独立内网 endpoint 时，默认使用虚拟主机式访问 | - | - | true |
...
| BACKEND_SIGNIN_VERIFICATION_EXPIRES_IN     | 登录验证码有效期。留空时沿用 BACKEND_EMAIL_CODE_EXPIRES_IN               | 10m              | -    | 10m                                              |
| BACKEND_SIGNIN_VERIFICATION_MAX_ATTEMPTS   | 单个登录验证码允许的错误次数，超出后验证码作废，需要重新获取                          | 5                | -    | 5                                                |
| **安全验证配置** |  |  |  |  |
...
| BACKEND_MAIL_INVITE_SPACE_NAME_MAX_LENGTH | 邀请邮件主题中空间或项目名称的最大长度。为空或 0 时不截断                   | -                | -    | 32                                               |
| **会话/JWT 配置** |  |  |  |  |
...
| TABLE_LIMIT_TABLES_PER_BASE_MAX           | 每个项目最大表数量                                      | 1000             | -    | 1000                                             |
...
| SANDBOX_JWT_SECRET                          | 沙箱会话签名密钥 —— 请按主机生成随机值，并在沙箱服务中使用相同的值 | - | 是 | 随机字符串 |
...
| V2_TABLE_QUERY_OPS_ENABLED                 | 是否开启表格查询运维；关闭时管理面板的 **表格查询运维** 没有数据，改动后需重启 | false | - | true |
| **分析监控** |  |  |  |  |
...
| PUBLIC_DATABASE_PROXY                      | 对外可达的数据库地址（host:port），项目「数据库连接」面板生成的连接凭据会使用该地址；配置后即可从外部进行只读 SQL 访问 | -               | -    | db.example.com:42345                              |
...
| BIG_TRANSACTION_TIMEOUT                    | 大型内部事务的超时时间（毫秒），例如导出大型项目                          | 600000          | -    | 1200000                                           |
