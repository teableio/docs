...
Quando l'analizzatore rileva numerose ricerche di sottostringhe in una Tabella, genera un unico suggerimento **GIN per testo generato** per l'intera Tabella, anziché un indice per ogni Campo. Tale riga offre **Crea percorso di ricerca** al posto di **Analisi approfondita** e **Aggiungi indice** non viene visualizzato. Il suggerimento si chiude automaticamente quando la Tabella dispone del percorso.

**Crea percorso di ricerca** apre una finestra che esegue un'analisi di prova (dry run) della Tabella senza lasciare la pagina. La finestra mostra il provider di indice del candidato, il numero di Campi coperti, le righe stimate e il costo `EXPLAIN` prima e dopo. Inserisci un termine di ricerca di esempio lungo almeno quanto la lunghezza minima indicata: serve solo alla verifica e non viene salvato. Su una Tabella grande, conferma anche la casella della finestra di manutenzione. Alla conferma Teable verifica di nuovo il candidato e attiva il percorso di ricerca.

Se l'analisi non produce candidati, la finestra ne spiega il motivo e non esegue nulla. Una Tabella molto larga viene rifiutata perché Teable non crea per essa un documento di ricerca su tutti i Campi, e un termine più lungo non cambia questo esito. Usa **Analizza i Campi selezionati** per passare all'area di lavoro di analisi e analizzare solo i Campi in cui gli utenti cercano davvero.
