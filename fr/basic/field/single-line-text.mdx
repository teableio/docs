...
Les adresses web contenues dans une cellule deviennent automatiquement des liens cliquables, sans aucun réglage. Sont reconnues les adresses commençant par `https://` ou `www.`, ainsi que les domaines courants comme `teable.ai`. Si une valeur contient plusieurs adresses, chacune devient un lien distinct. Dans un Enregistrement développé, un bouton situé près du champ de saisie répertorie toutes les adresses détectées dans la cellule.

Les noms de fichiers et les numéros de version tels que `readme.md` ou `v1.2.3` ne sont pas pris pour des adresses.

## Mode d’affichage

Texte sur une seule ligne prend en charge trois modes d’affichage :

| Mode d’affichage | Utilité | Comportement au clic |
| --- | --- | --- |
| Par défaut | Noms, titres, identifiants et autres textes bruts | Les adresses deviennent des liens et s’ouvrent au clic |
...
- **Gestion des coordonnées client** : créez un Champ « Site web de l’entreprise » et saisissez-y l’adresse afin que le site s’ouvre en cliquant dessus. Créez un Champ « Téléphone » et définissez-le sur **Téléphone** afin que les utilisateurs mobiles puissent appeler d’un seul geste.
...
- E-mail et Téléphone sont des modes d’affichage. Le Champ reste de type Texte sur une seule ligne et peut toujours être trié, filtré et traité avec des fonctions de texte.
