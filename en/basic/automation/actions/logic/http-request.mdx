...
When you insert dynamic values with **+**, Teable keeps the selected field path and any formatting options with the placeholder. This is useful when an API requires a specific output format.

Supported transformations include:

- JSON string conversion for values that need to be embedded safely in JSON.
- Date formatting for APIs that require a specific date or time format.

If the request body uses JSON, Teable escapes inserted values so the generated request stays valid JSON.
