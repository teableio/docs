...
`POST /api/automation/runtime/ai` envoie un prompt au modèle d’IA de votre projet. Le projet est déterminée par le contexte de l’automatisation ; aucun ID de projet n’est donc nécessaire. Le corps accepte `prompt`, ainsi que les paramètres facultatifs `attachments`, `modelKey`, `temperature` et `outputType` ; la réponse est `{ "message": ... }`.
