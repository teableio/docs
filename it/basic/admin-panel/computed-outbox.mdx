...
- Filtra per **Spazio**, **Progetto**, **Causa** o **Esito**, cerca un'attività, un Progetto, uno Spazio o un errore oppure imposta un tempo minimo di elaborazione per far emergere le attività lente.
...
| **Sequenza temporale** | Tre assi affiancati: **End-to-end** va dalla modifica sorgente alla convergenza delle scritture calcolate, **Questo task** copre dal claim al commit e **Job wakeup** è il wakeup che lo ha eseguito. L'asse del task è suddiviso in attesa in coda, claim / setup e calcolo. Il job di wakeup svuota anche altro lavoro scaduto sullo stesso database, quindi di norma dura più dell'end-to-end. |
| **Catena di attività** | Ogni attività della cascata, nidificata sotto quella che l'ha accodata, per mostrare l'origine di ciascun ramo. I batch parziali della stessa fase sono una coda seriale e restano sullo stesso livello invece di annidarsi ulteriormente. Ogni riga mostra stato, passaggi del piano, **Destinazione di scrittura** (la Tabella e i Campi aggiornati dall'attività), **Complessità stimata**, profondità della cascata, ora di accodamento e durata. |
...
Un gruppo contrassegnato come **Progetto eliminato** appartiene a un Progetto non più esistente. Il suo ripristino fallirebbe di nuovo immediatamente, pertanto **Elimina gruppo** è l'unica azione disponibile.
...
| **Limiti di acquisizione per Progetto** | Numero di attività di calcolo eseguite contemporaneamente per un Progetto (**Per Progetto**) e per una Tabella di origine al suo interno (**Per Tabella iniziale**). Entrambi hanno come valore predefinito 2 e accettano valori da 1 a 16. |

Aumenta i limiti di acquisizione quando il collo di bottiglia è una singola Progetto molto attiva, e la concorrenza per processo quando sono accodate contemporaneamente molte Progetti.
