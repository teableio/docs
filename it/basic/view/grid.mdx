...
Lo stesso pannello può segnalare anche questi errori:

| Messaggio | Che cosa significa | Che cosa fare |
| --- | --- | --- |
| `La compilazione della formula supera il limite di calcolo; i risultati non sono stati aggiornati` | La struttura espansa della formula supera il limite | Semplifica la formula o sposta i risultati intermedi in Campi separati; vedi [Formula](/it/basic/field/formula) |
| `I risultati non sono stati aggiornati a causa di un limite di risorse` | Il calcolo ha raggiunto un limite di risorse del database | Modifica meno record alla volta; se si ripete, contatta un amministratore |
| `Il calcolo non è terminato; alcuni risultati non sono stati aggiornati` | La catena di calcolo è troppo profonda per concludersi in un passaggio | Riduci i riferimenti a catena tra Campi, poi modifica di nuovo i dati di origine per avviare un nuovo calcolo |
