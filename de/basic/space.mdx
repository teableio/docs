...
## Ihre Spaces beim Löschen des Kontos

Öffnen Sie Ihr Avatar unten links → **Einstellungen** → **Mein Konto** und klicken Sie auf **Konto löschen**. Der Dialog listet zuerst die Spaces auf, deren einziger Besitzer Sie sind; für jeden davon ist eine Entscheidung nötig. Alle anderen Spaces verlieren lediglich Sie als Mitarbeitende und bleiben ansonsten unverändert.

- Standardmäßig wandert ein Space zusammen mit dem Konto in den Papierkorb, genau wie bei einer Löschung von Hand.
- Hat der Space weitere Mitglieder, können Sie ihn an eine dieser Personen übergeben; die Übergabe läuft vor dem Löschen des Kontos.
- Ein Space mit aktivem Abonnement kann nicht mit dem Konto in den Papierkorb. Kündigen Sie zuerst das Abonnement oder übergeben Sie den Space an ein anderes Mitglied.

Sind diese Spaces geklärt, geben Sie zur Bestätigung wie angefordert `DELETE` ein. Ein gelöschtes Konto lässt sich nicht wiederherstellen.

## Mit mir geteilt

Wenn jemand ein Projekt als Mitbearbeiter mit Ihnen teilt, wird das Projekt in der Seitenleiste des Space unter **Mit mir geteilt** angezeigt.

Mit Ihnen geteilte Projekte unterscheiden sich von Projekten in Ihren eigenen Spaces:

- Der Zugriff wird von der Person gesteuert, die das Projekt geteilt hat.
- Die verfügbaren Aktionen hängen von der Ihnen erteilten Projekt-Berechtigung ab.
- Wenn die teilende Person Ihren Zugriff entfernt, wird das Projekt nicht mehr angezeigt.

Informationen zur öffentlichen Freigabe von Projekten finden Sie unter [Projekt](/de/basic/base).
...
Auf der Seite **Chat in IM** können Space-Administratoren Bots erstellen und deren Modell, zugängliche Projekte, Skills, Dateien und IM-Integration konfigurieren.
