...
- **Proteggi le credenziali.** Le password SMTP vengono archiviate in forma crittografata in Teable, ma evita di condividere configurazioni di automazione contenenti credenziali sensibili. Quando una singola azione viene configurata con un proprio account SMTP, la sua password è un segreto concesso a quell'automazione; vedi [Invia email](/it/basic/automation/actions/communication/send-email-overview).
