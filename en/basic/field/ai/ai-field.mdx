...
- Choose the AI model.
- For **Generate Image** on an attachment field, open **Advanced settings** if you need to set image size, aspect ratio, resolution, quality, or the number of images. The available options depend on the selected image model.
- **Auto-update**
...
- Use a text-to-image model when the prompt only needs text input.
- Use an image-to-image capable model when the prompt should use existing image attachments as input.
- Use **Advanced settings** to adjust supported image options such as size, aspect ratio, resolution, quality, and output count.

If an option does not appear, the selected model does not expose that capability.
