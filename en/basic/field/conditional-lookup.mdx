...
<Steps>
  ...
  <Step title="Remove duplicate values (optional)">
    Turn on **Remove duplicate values** to keep each value only the first time it appears. Teable removes the duplicates after applying the filter, sorting, and record limit, so the limit still counts the matching records rather than the values left in the result.
  </Step>
</Steps>
