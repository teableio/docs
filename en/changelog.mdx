...
* **Improved computed field stability**: Fixed idle tables remaining stuck on “Checking calculation status” after Lookup or linked record updates.

* **Improved multi-level calculation accuracy**: Ensured downstream formulas update correctly after multi-step Lookup and computed field recalculation.

* **Optimized computed field performance**: Reduced unnecessary calculations, Lookup, and Rollup updates when no records require calculation or results remain unchanged.

* **Improved calculation status messaging**: Prevented false warnings, clarified empty tasks, and kept unresolved errors visible for accurate progress tracking.

* **Improved calculation activity reliability**: Displayed errors clearly and preserved existing status when follow-up checks fail or time out.

* **Prevented repeated invalid calculation tasks**: Stopped retries for tasks linked to deleted or missing Bases, improving system stability.

* **Improved shared document stability**: Enhanced collaboration, loading, and replay error handling, clarified permissions, and prevented cascading failures.
...
* **Improved all-field search performance**: Optimized large-table lists, counts, groups, and aggregations for numeric, rounded-value, and short-keyword searches.

* **Improved search responsiveness**: Statistical queries now stop when users modify or cancel searches, reducing waits and resource usage.

* **Improved restricted-user access**: Fixed permission errors for readable computed fields and improved reliability after permission changes or reconnection.
