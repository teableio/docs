---
title: "Docker 部署"
description: "使用 Docker Compose 部署 Teable，并配置常用部署选项。"
---
...
* 操作系统：建议使用 Linux 发行版，如 Ubuntu 20.04 LTS。
* 内存：至少 4GB RAM。
* CPU：至少 2 核。
* 磁盘空间：至少 40GB 的可用空间。
...
* 理解 Docker 和容器化原理的基本知识。
* 在您的机器上安装了 Docker 和 Docker Compose。访问官方 Docker 文档了解安装指南。
...
无法导入文件通常是因为没有正确配置 PUBLIC_ORIGIN 环境变量。您需要将 PUBLIC_ORIGIN 设置为当前可访问的地址：
```bash
# 本地开发环境
PUBLIC_ORIGIN=http://127.0.0.1:3000
