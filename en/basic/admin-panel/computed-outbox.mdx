...
A task whose source table is in the trash waits for that table to come back. Once the task is more than about a day old, Teable stops waiting and turns it into a dead letter of kind `obsolete_plan`. Restore the table before you recover the group: while it is still in the trash, a recovered task dead-letters the same way again. Discard the group when the table is not coming back.
