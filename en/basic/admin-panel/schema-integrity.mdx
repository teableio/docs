...
description: "Investigate and repair mismatches between a project's field definitions and its database structure."
---
...
An instance admin can check any project in the instance without joining its space first, tenants on a customer-managed database (BYODB) included.

## Run a Check

Find the project with the search box, by project, space, table id, or name. The result list shows the **Space** it belongs to, the **Data DB** it uses (default or BYODB), and its **Runtime** (v1 or v2), which is how you confirm you have the right one. The same project name in different spaces is common.
