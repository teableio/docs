...
Proteggi con una password le Viste, i Progetti o gli artefatti condivisi. Quando è abilitata, i destinatari devono inserire la password corretta prima di accedere ai contenuti condivisi.

Quando attivi la protezione con password, Teable genera automaticamente una password casuale, che è subito attiva. Fai clic su **Copia link e password** accanto alla password per copiare link e password in un'unica riga di testo da inviare direttamente a chi visita. Per cambiarla, fai clic sul pulsante di modifica e inserisci una nuova password, oppure fai clic su **Genera password casuale** per ottenerne un'altra.

La password è visibile in chiaro solo nel pannello di condivisione subito dopo averla impostata. Quando riapri le impostazioni di condivisione, la password viene nascosta; per copiarla di nuovo, impostane una nuova.

## Account e accesso

### Dispositivi di accesso

Apri l'avatar in basso a sinistra → **Impostazioni** → **Devices** per vedere su quali dispositivi è attualmente connesso il tuo account e la **Login history** degli ultimi 30 giorni. Per ogni dispositivo vengono mostrati il browser o l'app mobile, il sistema operativo, il metodo di accesso, l'indirizzo IP e l'ultima attività.

Se noti un dispositivo che non riconosci, fai clic su **Sign out** per quel dispositivo, poi cambia la password. Fai clic su **Sign out all other devices** per disconnettere in una volta sola tutti i dispositivi tranne quello attuale. I dispositivi disconnessi devono accedere di nuovo per continuare a usare Teable.

### Blocco dopo accessi non riusciti

Dopo 5 password errate consecutive per lo stesso indirizzo email, l'account viene bloccato per 15 minuti e ogni ulteriore tentativo errato in quel periodo viene rifiutato.
...
|--------|-------------|----------|
| **[Esportazione Progetto](/it/basic/base#export-base-backup-&-migration)** | Scarica l'intero Progetto come file `.tea` (struttura, dati, Automazioni) | Backup completo, migrazione |
| **[Duplicazione Progetto](/it/basic/base#duplicate-a-base-to-another-space)** | Crea una copia del Progetto all'interno di Teable | Snapshot rapido |
...
<Tip>
Puoi eseguire manualmente il backup dei Progetti esportando l'intero Progetto come file `.tea`, esportando singole Tabelle come file CSV oppure recuperando i dati tramite l'API di Teable.
</Tip>
