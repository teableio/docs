...
## Rilevamento automatico dei link

Gli indirizzi web contenuti in una cella diventano link cliccabili in automatico, senza impostare nulla. Vengono riconosciuti gli indirizzi che iniziano con `https://` o `www.` e i domini più comuni come `teable.ai`. Se un valore contiene più indirizzi, ciascuno diventa un link a sé. Nel Record espanso, un pulsante accanto al campo di inserimento elenca tutti gli indirizzi trovati nella cella.

Nomi di file e numeri di versione come `readme.md` o `v1.2.3` non vengono scambiati per indirizzi.

## Modalità di visualizzazione

Testo su una riga supporta tre modalità di visualizzazione:

| Modalità di visualizzazione | Utile per | Comportamento al clic |
| --- | --- | --- |
| Predefinita | Nomi, titoli, ID e altro testo normale | Gli indirizzi diventano link e si aprono al clic |
...
- **Gestione dei contatti dei clienti**: crea un Campo "Sito web aziendale" e digita l'indirizzo, in modo che il sito si apra con un clic. Crea un Campo "Telefono" e impostalo su **Telefono**, in modo che gli utenti mobili possano chiamare con un solo tocco.
...
- Email e Telefono sono modalità di visualizzazione. Il Campo rimane di tipo Testo su una riga e può continuare a essere ordinato, filtrato ed elaborato con funzioni di testo.
