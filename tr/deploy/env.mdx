...
| BACKEND_SIGNIN_VERIFICATION_EXPIRES_IN     | Oturum açma doğrulama kodunun geçerlilik süresi. Ayarlanmazsa BACKEND_EMAIL_CODE_EXPIRES_IN geçerlidir | 10m              | -    | 10m                                              |
| BACKEND_SIGNIN_VERIFICATION_MAX_ATTEMPTS   | Her oturum açma kodu için izin verilen hatalı deneme sayısı; bu sayının üzerinde kod geçersiz olur ve yenisi istenmelidir | 5                | -    | 5                                                |
...
| BACKEND_MAIL_INVITE_SPACE_NAME_MAX_LENGTH  | Davet e-postası konularında Alan veya Proje adının en fazla uzunluğu. Boş değer veya 0, kısaltmayı devre dışı bırakır | - | - | 32 |
...
| TABLE_LIMIT_TABLES_PER_BASE_MAX            | Proje başına en fazla tablo sayısı                                                   | 1000            | -        | 1000                                              |
...
| V2_TABLE_QUERY_OPS_ENABLED                 | Tablo Sorgusu İşlemleri'nin açılıp açılmayacağı; kapalıyken **Tablo Sorgusu İşlemleri** sayfasında veri bulunmaz. Değişiklikten sonra yeniden başlatın        | false           | -        | true                                              |
| **Analiz ve İzleme** |  |  |  |  |
...
| PUBLIC_DATABASE_PROXY                      | Projenin "Veritabanı Bağlantısı" panelinde oluşturulan kimlik bilgilerinde kullanılan, dışarıdan erişilebilir veritabanı adresi (ana bilgisayar:bağlantı noktası); harici salt okunur SQL erişimini etkinleştirmek için ayarlayın | -               | -        | db.example.com:42345                              |
...
| BIG_TRANSACTION_TIMEOUT                    | Büyük Projelerin dışa aktarılması gibi büyük dahili işlemler için zaman aşımı (ms) | 600000          | -        | 1200000                                           |
