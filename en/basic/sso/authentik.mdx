...
<Note>Replace `__providerId__` in the URL with the providerId shown after creating the authentication provider in Teable, or copy the callback URL directly from the Teable page.</Note>
...
<Warning>Store the Client Secret securely. You can regenerate it later if needed.</Warning>
...
<Note>Replace `{your-authentik-domain}` with your actual Authentik instance domain and `{application-slug}` with the slug you configured (e.g., `teable`).</Note>
