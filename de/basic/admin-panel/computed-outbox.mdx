...
- Filtern Sie nach **Space**, **Projekt**, **Ursache** oder **Ergebnis**, suchen Sie nach einer Aufgabe, einem Projekt, einem Space oder Fehler oder legen Sie eine Mindestverarbeitungszeit fest, um langsame Aufgaben anzuzeigen.
...
| **Zeitachse** | Drei Spuren nebeneinander: **Ende-zu-Ende** reicht von der Quelländerung bis zum Setzen der berechneten Schreibvorgänge, **Dieser Task** umfasst Claim bis Commit, und **Wakeup-Job** ist der Aufweckvorgang, der ihn ausgeführt hat. Die Task-Spur ist in Wartezeit, Claim / Setup und Berechnung unterteilt. Der Wakeup-Job arbeitet zusätzlich fällige Arbeit derselben Datenbank ab und dauert deshalb normalerweise länger als Ende-zu-Ende. |
| **Aufgabenkette** | Jede Aufgabe in der Kaskade, unter der Aufgabe verschachtelt, die sie eingereiht hat, damit Sie sehen können, woher ein Zweig stammt. Teilbatches derselben Stufe sind eine serielle Warteschlange und stehen flach auf einer Ebene, statt weiter verschachtelt zu werden. Jede Zeile zeigt ihren Zustand, Plan-Schritte, **Schreibziel** (die Tabelle und Felder, die diese Aufgabe aktualisiert), **Geschätzte Komplexität**, Kaskadentiefe, Einreihungszeit und Dauer. |
...
Eine als **Projekt gelöscht** markierte Gruppe gehört zu einem Projekt, das nicht mehr existiert. Ihre Wiederherstellung würde sofort erneut fehlschlagen, daher ist **Gruppe verwerfen** die einzige angebotene Aktion.
...
Eine Pause läuft immer selbstständig ab, und zwei Stunden sind die längste einstellbare Dauer. Wenn der Vorfall länger anhält, klicken Sie in der Zeile der Pause auf **Verlängern**, wählen Sie die zusätzliche Dauer und bestätigen Sie. Die neue automatische Fortsetzung wird ab dem Zeitpunkt der Bestätigung berechnet, ebenfalls bis maximal zwei Stunden, sodass der Space zwischendurch nicht freigegeben wird.
...
| **Beanspruchungslimits pro Projekt** | Wie viele berechnete Aufgaben gleichzeitig für ein Projekt (**Pro Projekt**) und für eine Quelltabelle darin (**Pro Seed-Tabelle**) ausgeführt werden. Beide haben standardmäßig den Wert 2 und akzeptieren Werte von 1 bis 16. |

Erhöhen Sie die Beanspruchungslimits, wenn ein ausgelastetes Projekt den Engpass bildet, und die Gleichzeitigkeit pro Prozess, wenn viele Projekte gleichzeitig in die Warteschlange eingereiht sind.
