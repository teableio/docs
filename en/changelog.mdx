...
# Compute Status and Stability Optimization

## Feature Updates

* Added visible calculation activity for computed fields, including formulas, lookups, and rollups.

## Fixes & Improvements

* **Improved table selection with search highlighting**: Selected rows or cells are easier to distinguish from search results.

* **Improved stats display in narrow columns**: Key values like percentages are prioritized to avoid misleading truncation.

* **Fixed grouped view field values for new records**: New records inherit the correct group value, even if the group field is hidden.

* **Improved calculated result updates after linking records**: Lookup and formula fields update faster after user info changes.

* **Fixed save failures when only editing field names or descriptions**: Edits no longer trigger unintended computed field backfill.

* **Improved editing stability for rollup and other computed fields**: Reduced errors when editing related field configurations.

* **Fixed lost configuration after renaming nested lookup fields**: Single-select options and displayed record values are preserved.

* **Improved attribution continuity**: Fixed attribution loss when moving between different Teable sites.

* **Improved Cookie handling stability**: Enhanced compatibility and stability in some deployment environments.
