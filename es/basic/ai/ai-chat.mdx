...
La memoria almacena la información que pides explícitamente a Cuppy que recuerde. Su ámbito se define por usuario y Proyecto: cada usuario de cada Proyecto tiene un contexto de memoria independiente, y la memoria guardada en otro Proyecto no aparece automáticamente en la actual.
...
<AccordionGroup>
  ...
  <Accordion title="¿Puede el Chat de IA trabajar en otro Proyecto?">
    Sí. La IA no está limitada al Proyecto a la que pertenece el chat; puede buscar y trabajar en las demás Proyectos a las que tienes acceso. Indical Proyecto en tu solicitud, por ejemplo: "compara esta Tabla con la Tabla de pedidos del Proyecto Ventas". `@` solo enumera los nodos del Proyecto actual, y cualquier acción para la que no indiques otro destino se ejecutará en el propio Proyecto del chat.
  </Accordion>
  ...
  <Accordion title="¿Se comparte la memoria entre Proyectos?">
    No. El ámbito de la memoria se define por usuario y Proyecto. Si quieres reutilizar la memoria de otro Proyecto, pide a Cuppy que lea la memoria de esa Proyecto y guarde las partes pertinentes en la actual.
  </Accordion>
