...
Le même panneau peut aussi signaler ces échecs :

| Message | Signification | Que faire |
| --- | --- | --- |
| `La compilation de la formule dépasse la limite de calcul ; les résultats n’ont pas été mis à jour` | La structure déployée de la formule dépasse la limite | Simplifiez la formule ou déportez les résultats intermédiaires dans des Champs distincts ; voir [Formule](/fr/basic/field/formula) |
| `Les résultats n’ont pas été mis à jour en raison d’une limite de ressources` | Le calcul a atteint une limite de ressources de la base de données | Modifiez moins d’enregistrements à la fois ; contactez un administrateur si cela se répète |
| `Le calcul n’est pas terminé ; certains résultats n’ont pas été mis à jour` | La chaîne de calcul est trop profonde pour aboutir en une passe | Réduisez les références en cascade entre Champs, puis modifiez à nouveau les données source pour relancer le calcul |
