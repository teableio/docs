...
| Serveur Email Personnalisé | Non | Envoie cette action via un compte SMTP précis, sans toucher aux autres automatisations |

\* Au moins l’un des champs **À** ou **CCI** est obligatoire.

**Limite de débit :** 5 e-mails/seconde/projet.
...
### Donner à cette action son propre SMTP

Pour envoyer cette seule action via un autre compte SMTP, cliquez sur **Ajouter Configuration** sous **Serveur Email Personnalisé**, puis renseignez l’adresse du serveur, le port, l’identifiant et les informations d’expéditeur.

Le mot de passe ne se saisit plus ici : cliquez sur **Sélectionner un secret** pour choisir l’un des secrets accordés à cette automatisation, ou sur **Ajouter un secret** pour en créer un (nom de variable par défaut `SMTP_PASSWORD` ; un deuxième compte SMTP dans le même workflow devient `SMTP_PASSWORD_2`). L’action conserve une référence : la valeur en clair n’entre pas dans la configuration du workflow, et l’historique d’exécution comme les exports l’affichent masquée.

**Réinitialiser** efface cette configuration et l’action revient au serveur de messagerie par défaut.

<Info>Cette boîte de dialogue n’envoie pas d’e-mail de test. Une fois la configuration terminée, exécutez le workflow une fois pour vérifier l’envoi.</Info>
