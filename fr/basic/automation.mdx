...
Une automatisation appelle les services externes avec vos identifiants : une connexion OAuth ou un secret que vous enregistrez. Ouvrez le workflow dans l’onglet d’édition, cliquez sur **Identifiants et configuration** à côté des contrôles de test, accordez un identifiant à cette automatisation et donnez-lui un **Nom de la variable**.

Les actions de script lisent ce nom dans une variable d’environnement telle que `process.env.EXTERNAL_API_KEY` ; dans les autres actions, **Insérer un secret** à côté d’un champ ajoute une référence. Les valeurs ne sont jamais affichées en clair et les résultats de test les masquent.

Les identifiants eux-mêmes se trouvent dans **Paramètres** → **Intégrations**, voir [Identifiants et intégrations](/fr/basic/credential).
...
| Élément | Limite |
|---|---|
| Débit d’envoi d’e-mails | 5 / seconde / projet |
| Débit de réception des webhooks | 50 / seconde / projet, 2 / seconde / workflow |
...
<AccordionGroup>
  <Accordion title="Pourquoi ai-je reçu une alerte d’utilisation des exécutions d’automatisation ?">
    ...
    La notification ouvre **Paramètres de l’Espace** → **Facturation**. Lorsqu’elles sont disponibles, elle indique quel workflow et quelle Projet ont contribué au plus grand nombre d’exécutions. Les rappels de quota sont basés sur les exécutions comptabilisées dans la limite de votre forfait ; les vérifications d’utilisation inhabituelle peuvent également prendre en compte les exécutions récentes avec l’état **success**, **failed** ou **pending**.
