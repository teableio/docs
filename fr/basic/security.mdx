...
Protégez vos vues, projets ou artefacts partagés par authentification par mot de passe. Lorsqu’elle est activée, les destinataires doivent saisir le mot de passe correct avant d’accéder au contenu partagé.

Lorsque vous activez la protection par mot de passe, Teable génère automatiquement un mot de passe aléatoire, effectif immédiatement. Cliquez sur **Copier le lien et le mot de passe** à côté du mot de passe pour copier le lien et le mot de passe sur une seule ligne, prête à être envoyée aux visiteurs. Pour le changer, cliquez sur le bouton de modification, puis saisissez un nouveau mot de passe ou cliquez sur **Générer un mot de passe aléatoire**.

Le mot de passe n’apparaît en clair dans le panneau de partage que juste après avoir été défini. Lorsque vous rouvrez les paramètres de partage, il est masqué ; pour le copier à nouveau, définissez un nouveau mot de passe.

## Compte et connexion

### Appareils connectés

Cliquez sur votre avatar en bas à gauche → **Paramètres** → **Devices** pour voir les appareils sur lesquels votre compte est actuellement connecté, ainsi que le **Login history** des 30 derniers jours. Chaque appareil affiche le navigateur ou l’application mobile, le système d’exploitation, la méthode de connexion, l’adresse IP et la dernière activité.

Si vous repérez un appareil inconnu, cliquez sur **Sign out** pour cet appareil, puis changez votre mot de passe. Cliquez sur **Sign out all other devices** pour déconnecter en une fois tous les appareils sauf celui que vous utilisez. Les appareils déconnectés doivent se reconnecter pour continuer.

### Verrouillage après échecs de connexion

Après 5 saisies erronées consécutives du mot de passe pour une même adresse e-mail, le compte est verrouillé pendant 15 minutes. Pendant ce délai, toute nouvelle tentative erronée est refusée.
...
|--------|-------------|----------|
| **[Exportation de Projet](/fr/basic/base#export-base-backup-&-migration)** | Télécharger tout le Projet sous forme de fichier `.tea` (structure, données, automatisations) | Sauvegarde complète, migration |
| **[Duplication de Projet](/fr/basic/base#duplicate-a-base-to-another-space)** | Créer une copie d’un Projet dans Teable | Instantané rapide |
...
<Tip>
Vous pouvez sauvegarder manuellement vos Projets en exportant tout le Projet sous forme de fichier `.tea`, en exportant des tables individuelles sous forme de fichiers CSV ou en récupérant vos données via l’API Teable.
</Tip>
