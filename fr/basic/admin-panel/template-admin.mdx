...
description: "Gérez les modèles de flux de travail de votre organisation afin de permettre aux utilisateurs de démarrer rapidement avec des Projets préconfigurées."
---
...
* **Solution** : lorsque le modèle est marqué ainsi, son utilisation ouvre d’abord la configuration des intégrations au lieu de copier directement le projet. La colonne **Integrations** indique les plateformes auxquelles il se connecte
* **Source** : le Projet d’origine utilisée pour créer ce modèle

#### Opérations sur les modèles

* **Nouveau modèle** : crée une nouvelle configuration de modèle
* **Publier un instantané** : capture l’état actuel du Projet source. Vous devez publier un instantané avant de pouvoir répertorier le modèle.
...
2. **Importer** : importez le fichier `.tea` en tant que nouveau Projet dans votre Espace
3. **Créer un modèle** : accédez à Administration des modèles > Nouveau modèle
4. **Sélectionner la source** : choisissez le Projet importée comme source
