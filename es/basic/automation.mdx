...
Una automatización llama a los servicios externos con tus credenciales: una conexión OAuth o un secreto que guardas. Abre el flujo de trabajo en la pestaña de edición, haz clic en **Credenciales y configuración** junto a los controles de prueba, concede una credencial a esta automatización y ponle un **Nombre de la variable**.

Las acciones de código leen ese nombre desde una variable de entorno como `process.env.EXTERNAL_API_KEY`; en las demás acciones, **Insertar secreto** junto a un campo añade una referencia. Los valores nunca se muestran sin formato y los resultados de prueba los enmascaran.

Las credenciales se gestionan en **Ajustes** → **Integraciones**; consulta [Credenciales e integraciones](/es/basic/credential).
...
| Elemento | Límite |
|---|---|
| Frecuencia de envío de correos electrónicos | 5 por segundo y por Proyecto |
| Frecuencia de recepción de webhooks | 50 por segundo y por Proyecto; 2 por segundo y por flujo de trabajo |
...
<AccordionGroup>
  <Accordion title="¿Por qué he recibido una alerta sobre el uso de ejecuciones de automatización?">
    ...
    La notificación abre **Ajustes del Espacio** → **Facturación**. Cuando está disponible, muestra qué flujo de trabajo y Proyecto aportaron más ejecuciones. Los recordatorios de cuota se basan en las ejecuciones que cuentan para el límite del plan; las comprobaciones de uso inusual también pueden tener en cuenta las ejecuciones recientes con estado **correcto**, **fallido** o **pendiente**.
