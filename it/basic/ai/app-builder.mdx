...
<Note>
Usa App Builder per creare interfacce personalizzate e app autonome. Per l'analisi interattiva dei dati e le domande estemporanee all'interno del Progetto, usa [AI Chat](/it/basic/ai/ai-chat).
</Note>
...
1. **Dal Progetto**: in qualsiasi Progetto, fai clic su «Crea app» per generare un'applicazione collegata ai dati correnti.
...
- **Richieste di credenziali**: quando l'app ha bisogno di accedere a un account di terze parti o a un segreto durante l'esecuzione, App Builder fissa una scheda nel pannello della chat, per esempio **Questa app ha bisogno della tua connessione Slack**. Fai clic su **Collega il mio account** per avviare un flusso OAuth; la nuova connessione viene concessa subito a questa app. Se possiedi già una connessione o un segreto utilizzabile, ti basta concederlo: non serve collegarsi di nuovo. Con **Salta** non fornisci la credenziale e l'IA continua con le parti che può completare senza di essa.
- **Skill**: apri il menu **+** e scegli **Skill** per gestire le skill della conversazione di questa app. Una skill aggiunta qui è disponibile soltanto per l'app corrente; qui è inoltre possibile utilizzare le skill condivise con il Progetto e lo Spazio dell'app. Digita `/` nella casella di input per scegliere una skill abilitata.
...
L'archivio contiene nella radice un file `.env` con le sole variabili di runtime iniettate dalla piattaforma, token di accesso compreso; le credenziali concesse all'app non ne fanno parte. Esamina il contenuto prima di condividere il file ZIP.
...
- Le variabili personalizzate di un file `.env` nella radice del file ZIP vengono salvate come tuoi segreti e concesse all'app con gli stessi nomi. Le altre varianti `.env*`, come `.env.local`, vengono ignorate.
</Note>

#### Credenziali e configurazione

Non scrivere chiavi API, credenziali e valori di configurazione nel codice sorgente. Un'app usa **le tue credenziali**: una connessione OAuth o un segreto che salvi. Nel pannello di chat dell'App Builder apri **+** → **Altro** → **Credenziali e configurazione** e concedi una credenziale all'app.

| Dettaglio | Comportamento |
| --- | --- |
| Disponibilità | Le credenziali concesse all'app sono disponibili nell'ambiente di anteprima e nell'app pubblicata |
| Nome della variabile | L'alias che inserisci al momento della concessione; inizia con una lettera maiuscola e accetta solo lettere maiuscole, cifre e trattini bassi |
| Leggere un segreto | Il codice lato server legge variabili di ambiente come `process.env.MY_API_KEY` |
| Leggere una connessione | Il codice lato server chiama `getConnectionToken('ALIAS')` per ottenere un token di accesso |
| Valori salvati | Dopo il salvataggio i valori sono di sola scrittura; inserisci un nuovo valore per sostituirne uno |

Dopo aver aggiunto o rimosso una concessione, oppure sostituito il valore di una credenziale, l'ambiente di anteprima dell'app usa il nuovo valore. L'app pubblicata lo usa solo dopo una nuova pubblicazione.

La copia di un'app non copia le sue credenziali: nel pannello restano segnaposto **Non associato** e con **Associa la mia** inserisci la tua. Per la gestione delle credenziali e per sapere dove sono usate, vedi [Credenziali e integrazioni](/it/basic/credential).
...
Il repository contiene soltanto il codice sorgente. Teable non invia mai file il cui nome inizia con `.env`: né nella radice del progetto, né in una sottocartella, né nella cronologia dei commit. I valori di ambiente provengono dalle credenziali concesse all'app, risiedono in **Credenziali e configurazione** e non entrano mai in git. Il file `.gitignore` del repository elenca già `.env*`, pertanto un clone locale non ne eseguirà accidentalmente il commit.
...
Viene copiato solo ciò che la piattaforma inietta come variabili di runtime: endpoint API, token di accesso e variabili per l'accesso IA. Sotto il pulsante compare la nota **Solo le variabili di runtime iniettate dalla piattaforma: le credenziali concesse non sono incluse**. Le credenziali concesse all'app non escono da Teable: se servono all'esecuzione locale, aggiungi tu i valori nel file `.env`.
...
Per aggiungere o sostituire una credenziale usata dall'app in produzione, usa **Credenziali e configurazione** in Teable: modificare il file locale o sottoporre a commit un `.env` su GitHub non ha effetto. Quando cambia una variabile di piattaforma, ad esempio dopo aver rigenerato il token di accesso, copia di nuovo le variabili per aggiornare il file `.env` locale.
...
Alla scadenza del piano Business dello Spazio, Teable avvisa i proprietari dello Spazio che il dominio personalizzato smetterà presto di essere risolto. Rinnova prima di quella data per mantenere il dominio. Senza rinnovo, il dominio personalizzato smette di essere risolto e il link pubblico dell'app torna all'indirizzo predefinito di Teable. Se in seguito passi di nuovo al piano Business, dovrai associare di nuovo manualmente il dominio personalizzato.
...
App Builder salva le modifiche di ogni giro come una nuova versione. Quando il salvataggio non riesce, nella chat compare una scheda.

<Warning>
Le modifiche del giro non riuscito restano solo nella sandbox: vanno perse quando la sandbox viene rilasciata e non vengono salvate nemmeno rigenerando la risposta. Intervieni sulla scheda per tempo.
</Warning>

- **Salvataggio della versione non riuscito**: questa versione non è stata salvata. Lascia proseguire App Builder: il primo salvataggio riuscito porterà con sé anche queste modifiche.
- **Modifiche troppo grandi per il salvataggio**: questo giro supera il limite di dimensione per il salvataggio. Fai clic su **Invia all'IA** nella scheda: App Builder cerca artefatti di test, file di debug o allegati troppo grandi, li ripulisce e salva di nuovo. Quei file possono essere già finiti nella cronologia dei commit non salvati, quindi cancellarli può non bastare: lascia che sia App Builder a sistemare anche quella.
