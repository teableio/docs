...
<Note>
Utilisez le Créateur d’applications pour créer des interfaces personnalisées et des applications autonomes. Pour l’analyse interactive de données et les questions ponctuelles dans le Projet, utilisez [Chat IA](/fr/basic/ai/ai-chat).
</Note>
...
1. **Depuis un Projet** : dans n’importe quel Projet, cliquez sur « Créer une application » pour générer une application liée à vos données actuelles.
...
- **Demandes d’identifiants** : lorsque l’application a besoin d’accéder à un compte tiers ou à un secret à l’exécution, le Créateur d’applications épingle une carte dans le panneau de chat, par exemple **Cette application a besoin de votre connexion Slack**. Cliquez sur **Connecter mon compte** pour lancer un parcours OAuth ; la nouvelle connexion est aussitôt accordée à cette application. Si vous disposez déjà d’une connexion ou d’un secret utilisable, accordez-le simplement : inutile de vous reconnecter. **Ignorer** ne fournit pas l’identifiant ; l’IA poursuit les parties qu’elle peut réaliser sans lui.
- **Compétences** : ouvrez le menu **+** et choisissez **Compétences** pour gérer les compétences de cette conversation d’application. Une compétence ajoutée ici est disponible uniquement pour l’application actuelle ; les compétences partagées avec le Projet et l’Espace de l’application peuvent également être utilisées ici. Saisissez `/` dans le champ de saisie pour choisir une compétence activée.
...
L’archive contient à la racine un fichier `.env` qui ne comporte que les variables d’exécution injectées par la plateforme, jeton d’accès compris ; les identifiants accordés à l’application n’y figurent pas. Examinez le contenu avant de partager le ZIP.
...
- Les variables personnalisées d’un fichier `.env` à la racine du ZIP sont enregistrées comme vos secrets et accordées à l’application sous les mêmes noms. Les autres variantes `.env*`, telles que `.env.local`, sont ignorées.
</Note>

#### Identifiants et configuration

N’écrivez pas les clés API, les identifiants ni les valeurs de configuration dans le code source. Une application utilise **vos identifiants** : une connexion OAuth ou un secret que vous enregistrez. Dans le panneau de chat de l’App Builder, ouvrez **+** → **Plus** → **Identifiants et configuration**, puis accordez un identifiant à l’application.

| Détail | Comportement |
| --- | --- |
| Disponibilité | Les identifiants accordés à l’application sont disponibles dans l’environnement de prévisualisation et dans l’application publiée |
| Nom de la variable | L’alias saisi lors de l’autorisation ; il commence par une majuscule et n’accepte que majuscules, chiffres et traits de soulignement |
| Lire un secret | Le code d’application côté serveur lit des variables d’environnement telles que `process.env.MY_API_KEY` |
| Lire une connexion | Le code d’application côté serveur appelle `getConnectionToken('ALIAS')` pour obtenir un jeton d’accès |
| Valeurs enregistrées | Les valeurs sont accessibles en écriture seule après l’enregistrement ; saisissez une nouvelle valeur pour en remplacer une |

Après l’ajout ou le retrait d’une autorisation, ou le remplacement d’une valeur, l’environnement de prévisualisation de l’application utilise la nouvelle valeur. L’application publiée ne l’utilise qu’après une nouvelle publication.

Copier une application ne copie pas ses identifiants : le panneau conserve des emplacements **Pas encore lié**, et **Lier le mien** y place les vôtres. Pour la gestion des identifiants et le suivi de leur utilisation, voir [Identifiants et intégrations](/fr/basic/credential).
...
Le contenu copié se limite aux variables d’exécution injectées par la plateforme : point de terminaison API, jeton d’accès et variables d’accès à l’IA. Sous le bouton figure la mention **Uniquement les variables d'exécution injectées par la plateforme — les identifiants accordés ne sont pas inclus**. Les identifiants accordés à l’application ne quittent pas Teable ; si l’exécution locale en a besoin, ajoutez vous-même ces valeurs dans le fichier `.env`.
...
Pour ajouter ou remplacer un identifiant utilisé par l’application en production, passez par **Identifiants et configuration** dans Teable ; modifier le fichier local ou valider un `.env` dans GitHub reste sans effet. Après le changement d’une variable de plateforme, par exemple un jeton d’accès régénéré, copiez à nouveau les variables pour actualiser votre fichier `.env` local.
...
Lorsque l’offre Business de l’espace expire, Teable prévient les propriétaires de l’espace que le domaine personnalisé cessera bientôt d’être résolu. Renouvelez avant cette date pour conserver le domaine. Sans renouvellement, le domaine personnalisé cesse d’être résolu et le lien public de l’application revient à l’adresse Teable par défaut. Si vous repassez ensuite à l’offre Business, vous devrez associer à nouveau le domaine personnalisé manuellement.
...
Le Créateur d’applications enregistre les modifications de chaque tour comme une nouvelle version. Quand l’enregistrement échoue, une carte apparaît dans la conversation.

<Warning>
Les modifications du tour concerné ne subsistent que dans le bac à sable ; elles sont perdues quand celui-ci est recyclé, et régénérer la réponse ne les enregistre pas davantage. Traitez la carte sans tarder.
</Warning>

- **Échec de l’enregistrement de la version** : cette version n’a pas été enregistrée. Laissez le Créateur d’applications continuer : le prochain enregistrement réussi emportera aussi ces modifications.
- **Modifications trop volumineuses pour être enregistrées** : ce tour dépasse la limite de taille à l’enregistrement. Cliquez sur **Envoyer à l’IA** sur la carte : le Créateur d’applications recherche les artefacts de test, fichiers de débogage ou pièces jointes trop volumineux, fait le ménage, puis enregistre de nouveau. Ces fichiers peuvent déjà figurer dans l’historique des commits non enregistrés ; les supprimer ne suffit pas toujours, laissez le Créateur d’applications nettoyer aussi cet historique.
