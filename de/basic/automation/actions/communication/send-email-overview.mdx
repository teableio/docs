...
| Benutzerdefinierter E-Mail-Server | Nein | Versendet diese Aktion über ein bestimmtes SMTP-Konto, ohne andere Automatisierungen zu berühren |

\* Mindestens eines der Felder **An** oder **BCC** ist erforderlich.

**Ratenbegrenzung:** 5 E-Mails/Sekunde/Projekt.
...
### Dieser Aktion ein eigenes SMTP geben

Um diese eine Aktion über ein anderes SMTP-Konto zu versenden, klicken Sie unter **Benutzerdefinierter E-Mail-Server** auf **Konfiguration hinzufügen** und tragen Serveradresse, Port, Benutzernamen und Absenderdaten ein.

Das Passwort wird nicht mehr eingetippt: Klicken Sie auf **Secret auswählen**, um eines der dieser Automatisierung freigegebenen Secrets zu wählen, oder auf **Secret hinzufügen**, um eines anzulegen (Standardvariablenname `SMTP_PASSWORD`; ein zweites SMTP-Konto im selben Workflow wird zu `SMTP_PASSWORD_2`). Die Aktion speichert nur eine Referenz, der Klartext gelangt nicht in die Workflow-Konfiguration, und Laufverlauf wie Export zeigen ihn maskiert.

**Zurücksetzen** leert die Konfiguration, und die Aktion nutzt wieder den Standard-Mailserver.

<Info>Dieser Dialog verschickt keine Testmail. Führen Sie den Workflow nach der Konfiguration einmal aus, um den Versand zu prüfen.</Info>
