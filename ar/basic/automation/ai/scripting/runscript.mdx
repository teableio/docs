...
يرسل `POST /api/automation/runtime/ai` مطالبة إلى نموذج الذكاء الاصطناعي في مشروعك. يُحدَّد المشروع من سياق الأتمتة، ولذلك لا يلزم معرّف مشروع. يتلقى النص `prompt`، بالإضافة إلى `attachments` و`modelKey` و`temperature` و`outputType` الاختيارية؛ وتكون الاستجابة `{ "message": ... }`.
