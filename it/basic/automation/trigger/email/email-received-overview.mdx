...
|---|---|---|
| Tipo di connessione | Sì | Solo IMAP |
...
Il trigger si collega via IMAP, quindi funziona con qualsiasi provider che supporti il protocollo: server di posta self-hosted, email aziendali, provider come Yahoo o Zoho e anche Gmail e Outlook una volta abilitato IMAP sull'account.
...
| Password | Sì | Un segreto concesso a questa automazione, non un valore digitato nel trigger |
| Casella | No | Cartella da monitorare (valore predefinito: `INBOX`) |

Per la password, fai clic su **Seleziona un segreto** e scegli uno dei segreti concessi a questa automazione, oppure su **Aggiungi segreto** per crearne uno (nome variabile predefinito `IMAP_PASSWORD`). Conserva lì la password per l'app o il codice di autorizzazione, non la password di accesso alla casella; il trigger mantiene soltanto il riferimento al segreto.
...
4. Inserisci host, porta e nome utente del server IMAP, poi seleziona o aggiungi il segreto che contiene la password.
...
- **Gmail: errore "App meno sicura":** Gmail non consente l'accesso con una semplice password. Conserva invece una **Password per l'app** come segreto. Consulta [Configurare Gmail IMAP](/it/basic/automation/trigger/email/gmail-imap) per le istruzioni.
...
- Un trigger che conserva una password digitata in passato continua a funzionare e il campo mostra **Password impostata. Scegli un secret per sostituirla.** Alla prossima modifica quella password può diventare solo un segreto.
...
- [Credenziali e integrazioni](/it/basic/credential) — gestire i segreti e concederli a un'automazione
