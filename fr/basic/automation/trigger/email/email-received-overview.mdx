...
|---|---|---|
| Type de connexion | Oui | IMAP uniquement |
...
Le déclencheur se connecte en IMAP et fonctionne donc avec tous les fournisseurs qui prennent en charge le protocole : serveurs auto-hébergés, messageries d’entreprise, fournisseurs tels que Yahoo ou Zoho, ainsi que Gmail et Outlook dès que l’IMAP est activé sur le compte.
...
| Mot de passe | Oui | Un secret accordé à cette automatisation, et non une valeur saisie dans le déclencheur |
| Boîte aux lettres | Non | Dossier à surveiller (`INBOX` par défaut) |

Pour le mot de passe, cliquez sur **Sélectionner un secret** afin de choisir l’un des secrets accordés à cette automatisation, ou sur **Ajouter un secret** pour en créer un (nom de variable par défaut `IMAP_PASSWORD`). Enregistrez-y le mot de passe d’application ou le code d’autorisation, et non le mot de passe de connexion à la messagerie ; le déclencheur ne conserve que la référence au secret.
...
4. Saisissez l’hôte, le port et le nom d’utilisateur de votre serveur IMAP, puis sélectionnez ou ajoutez le secret qui contient le mot de passe.
...
- **Gmail : erreur « Application moins sécurisée » :** Gmail n’autorise pas la connexion avec un simple mot de passe. Enregistrez plutôt un **mot de passe d’application** comme secret. Consultez [Configurer Gmail IMAP](/fr/basic/automation/trigger/email/gmail-imap) pour obtenir des instructions.
...
- Un déclencheur qui conserve un mot de passe saisi auparavant continue de fonctionner, et le champ affiche **Mot de passe défini. Choisissez un secret pour le remplacer.** Le prochain changement de ce mot de passe devra passer par un secret.
...
- [Identifiants et intégrations](/fr/basic/credential) — gérer les secrets et les accorder à une automatisation
