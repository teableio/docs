---
title: "OIDC 单点登录"
description: "Teable 支持 OIDC 单点登录，通过 OIDC 单点登录，您可以方便的将 Teable 与外部系统进行集成，实现用户认证和授权。"
---
...
* `BACKEND_OIDC_CALLBACK_URL`：Teable 的回调 URL。设置为 https\://app.teable.ai/api/auth/oidc/callback。确保这与在 OIDC 提供商处注册的回调 URL 一致。
