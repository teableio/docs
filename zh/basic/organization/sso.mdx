...
<Card title="注意" horizontal>
  私有部署需要在环境变量中声明 `SSO_PROVIDER` 才能开启对应的单点登录功能
  - `SSO_PROVIDER` 的值为 `feishu` 时，会开启飞书单点登录功能
  - `SSO_PROVIDER` 的值为 `oidc` 时，会开启标准OIDC单点登录功能
  
  开启多个单点登录时，使用逗号分隔，需要将 `SSO_PROVIDER` 的值设置为 `feishu,oidc` 这样会开启飞书和标准OIDC单点登录功能
</Card>
