...
# Formula fields and stability updates

## Feature Updates

* Added visible calculation activity status for formulas, lookups, and rollups, clarifying when table values are still calculating.
...
* **Improved statistics display in narrow columns**: Key values like percentages are prioritized to reduce misleading truncation.

* **Fixed grouped view defaults for new records**: New records inherit the correct group value even when the grouped field is hidden.

* **Improved calculation updates after linking records**: Lookup and formula fields update faster after creating linked records following user info updates.

* **Fixed save failures when only editing field names or descriptions**: Editing names or descriptions no longer triggers calculation backfill.
...
* **Improved promotion attribution continuity**: Fixed attribution loss when navigating between different Teable sites.

* **Improved Cookie handling stability**: Enhanced Cookie compatibility and reliability in certain deployment environments.
