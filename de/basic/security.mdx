...
Schützen Sie Ihre freigegebenen Ansichten, Projekte oder Artefakte mit Passwortauthentifizierung. Wenn sie aktiviert ist, müssen Empfänger das richtige Passwort eingeben, bevor sie auf den freigegebenen Inhalt zugreifen können.

Wenn Sie den Passwortschutz aktivieren, generiert Teable automatisch ein zufälliges Passwort, das sofort gilt. Klicken Sie neben dem Passwort auf **Link und Passwort kopieren**, um Link und Passwort als eine Textzeile zu kopieren und direkt an die Besucher zu senden. Zum Ändern klicken Sie auf die Schaltfläche zum Bearbeiten und geben ein neues Passwort ein, oder Sie klicken auf **Zufälliges Passwort generieren**.

Das Passwort ist nur direkt nach dem Festlegen im Freigabebereich im Klartext sichtbar. Wenn Sie die Freigabeeinstellungen erneut öffnen, ist es verborgen. Um es erneut zu kopieren, legen Sie ein neues Passwort fest.

## Konto und Anmeldung

### Angemeldete Geräte

Öffnen Sie über Ihren Avatar unten links **Einstellungen** → **Devices**, um zu sehen, auf welchen Geräten Ihr Konto derzeit angemeldet ist, sowie den **Login history** der letzten 30 Tage. Für jedes Gerät werden Browser oder mobile App, Betriebssystem, Anmeldemethode, IP-Adresse und die letzte Aktivität angezeigt.

Wenn Sie ein Gerät nicht kennen, klicken Sie bei diesem Gerät auf **Sign out** und ändern Sie anschließend Ihr Passwort. Mit **Sign out all other devices** melden Sie alle Geräte außer dem aktuellen auf einmal ab. Abgemeldete Geräte müssen sich erneut anmelden, bevor sie weiter verwendet werden können.

### Sperre nach fehlgeschlagenen Anmeldungen

Nach 5 aufeinanderfolgenden falschen Passworteingaben für dieselbe E-Mail-Adresse wird das Konto für 15 Minuten gesperrt. In dieser Zeit werden weitere falsche Versuche abgelehnt.
...
|--------|-------------|----------|
| **[Projekt-Export](/de/basic/base#export-base-backup-&-migration)** | Gesamtes Projekt als `.tea`-Datei herunterladen (Struktur, Daten, Automatisierungen) | Vollständige Sicherung, Migration |
| **[Projekt duplizieren](/de/basic/base#duplicate-a-base-to-another-space)** | Eine Kopie des Projekts innerhalb von Teable erstellen | Schnappschuss |
...
<Tip>
Sie können Ihre Projekte manuell sichern, indem Sie das gesamte Projekt als `.tea`-Datei exportieren, einzelne Tabellen als CSV-Dateien exportieren oder Ihre Daten über die Teable API abrufen.
</Tip>
