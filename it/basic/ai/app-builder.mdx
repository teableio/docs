...
Il contenuto copiato corrisponde a ciò che l'app riceve nella sandbox di Teable: le credenziali concesse all'app e le variabili inserite dalla piattaforma per l'endpoint API, il token di accesso e l'accesso IA dell'app.
...
Per aggiungere o sostituire una credenziale, usa **Credenziali e configurazione** in Teable anziché il file locale. Un file `.env` sottoposto a commit su GitHub non crea né aggiorna una credenziale. Dopo aver modificato una credenziale, copia nuovamente le variabili per aggiornare il file `.env` locale.
