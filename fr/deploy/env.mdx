...
| BACKEND_SIGNIN_VERIFICATION_EXPIRES_IN     | Durée de validité du code de vérification de connexion. À défaut, BACKEND_EMAIL_CODE_EXPIRES_IN s'applique | 10m              | -    | 10m                                              |
| BACKEND_SIGNIN_VERIFICATION_MAX_ATTEMPTS   | Nombre d'essais erronés autorisés par code de connexion ; au-delà, le code est invalidé et il faut en demander un nouveau | 5                | -    | 5                                                |
...
| BACKEND_MAIL_INVITE_SPACE_NAME_MAX_LENGTH | Longueur maximale du nom de l’Espace ou du Projet dans les objets des e-mails d’invitation. Une valeur vide ou 0 désactive la troncature | - | - | 32 |
...
| TABLE_LIMIT_TABLES_PER_BASE_MAX | Nombre maximal de Tables par Projet | 1000 | - | 1000 |
...
| V2_TABLE_QUERY_OPS_ENABLED | Indique s’il faut activer les opérations de requêtes de Table ; désactivées, la page **Opérations de requêtes de Table** n’affiche aucune donnée. Redémarrez après modification | false | - | true |
...
| PUBLIC_DATABASE_PROXY | Adresse de base de données accessible de l’extérieur (hôte:port) utilisée dans les identifiants générés par le panneau « Connexion à la base de données » du projet ; définissez-la pour activer l’accès SQL externe en lecture seule | - | - | db.example.com:42345 |
...
| BIG_TRANSACTION_TIMEOUT | Délai d’expiration (ms) des grandes transactions internes, comme l’exportation de grands Projets | 600000 | - | 1200000 |
