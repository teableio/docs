...
<Steps>
  ...
  <Step title="Remove duplicate values">
    Turn on **Remove duplicate values** and each value is shown once, in its original order. Deduplication happens after the filter, sorting, and record limit, so the limit counts matching records rather than the values left in the result.
  </Step>
</Steps>
