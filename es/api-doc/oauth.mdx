...
Normalmente, el primer paso después de obtener un token es recuperar todas los Proyectos accesibles para el usuario actual:
...
Este endpoint devuelve todas los Proyectos a las que el usuario actual tiene permiso de acceso. Puedes usar el `baseId` de la respuesta para las llamadas posteriores a la API.
