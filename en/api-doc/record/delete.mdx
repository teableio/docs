...
<CodeGroup>
  ```bash CURL
  curl -X DELETE 'https://app.teable.io/table/__tableId__/record?recordIds[]=rec123456&recordIds[]=rec789012' \
  ...
  ```ts TypeScript
  const response = await fetch('https://app.teable.io/table/__tableId__/record?recordIds[]=rec123456&recordIds[]=rec789012', {
    method: 'DELETE',
    headers: {
  ...
  response = requests.delete(
      'https://app.teable.io/table/__tableId__/record',
      headers={
          'Authorization': 'Bearer __token__'
      },
      params={
