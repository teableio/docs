...
- While Teable looks like Excel, it's stricter. For example, **a "Number" column cannot contain text**. This constraint ensures data integrity and is the foundation for advanced filtering and automation. See [Teable vs Excel](/en/compare/teable-vs-excel) for details.
- Before deleting a table, check whether it is referenced by **linked fields** in other tables. Deleting the source table may cause data display issues in linked tables.
