...
| INT       | 返回逻辑参数为真时的number1，否则返回number2。也可用于制作嵌套IF语句。也可用于检查单元格是否为空/是否为空。 | `value`                   | Number | `INT(1.9) => 1` `INT(-1.9) => -2`                   |
...
| 函数名称      | 描述                                                                                         | 输入                                            | 输出                                      | 示例                                                                                                                                |
| --------- | ------------------------------------------------------------------------------------------ | --------------------------------------------- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| IF        | 如果逻辑参数为真，则返回`value1`，否则返回`value2`。还可用于制作嵌套的IF语句，以及检查单元格是否为空。                               | `logical, value1, value2`                     | String \| Number \| Boolean \| Datetime | `IF(2 > 1, "A", "B") => "A"` `IF(2 > 1, TRUE, FALSE) => TRUE`                                     |
...
| BLANK     | 返回一个空值。                                                                                    | `-`                                           | null                                    | `BLANK() => null` `IF(2 > 3, "Yes", BLANK()) => null`                                                 |
...
| IS\_AFTER      | 确定date1是否晚于date2。如果是，则返回true，否则返回false。                | `date1, date2, [unit]`       | Boolean  | `IS_AFTER("2023-09-10", "2023-09-08") => true` `IS_AFTER("2023-09-10", "2023-09-08", "month") => false` |
