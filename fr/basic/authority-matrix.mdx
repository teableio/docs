---
title: "Présentation"
description: "Définissez des périmètres d’accès pour différentes équipes dans le même projet, afin que les membres ne puissent voir, modifier ou exporter que les données dont ils ont besoin."
---

{/* mintlify-resync: 2026-05-28 */}
<Tip>Disponible avec le forfait Business et les forfaits supérieurs</Tip>

Lorsque les équipes commerciales, financières, opérationnelles et de direction partagent le même projet,
les autorisations déterminent qui peut accéder au projet, quelles données chacun peut consulter ou modifier,
...
| Champs sensibles dans un projet partagé | Les membres peuvent consulter les enregistrements de commande sans voir les champs de paiement ou de coût. |
...
- L’attribution d’un rôle depuis `Ajouter un utilisateur` ou `Ajouter depuis l’organisation` peut ajouter les
  membres ou départements sélectionnés comme collaborateurs du projet. La Matrice d’autorité limite ensuite
  ce à quoi ils peuvent accéder à l’intérieur du projet.

<Note>Pour en savoir plus sur les niveaux d’autorisation de projet des Espaces et des projets, consultez les [Autorisations de collaboration](/fr/basic/space/space-permission).</Note>
