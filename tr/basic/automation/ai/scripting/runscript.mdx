...
`POST /api/automation/runtime/ai`, Projenizin AI modeline bir istem gönderir. Proje otomasyon bağlamından alındığı için Proje kimliği gerekmez. Gövde `prompt` değerini ve isteğe bağlı `attachments`, `modelKey`, `temperature` ve `outputType` değerlerini alır; yanıt `{ "message": ... }` biçimindedir.
