...
| BACKEND_SIGNIN_VERIFICATION_EXPIRES_IN     | مدة صلاحية رمز التحقق عند تسجيل الدخول. إذا لم يُضبط فتُستخدم BACKEND_EMAIL_CODE_EXPIRES_IN | 10m              | -    | 10m                                              |
| BACKEND_SIGNIN_VERIFICATION_MAX_ATTEMPTS   | عدد المحاولات الخاطئة المسموح بها لكل رمز تسجيل دخول، وبعدها يُلغى الرمز ويلزم طلب رمز جديد | 5                | -    | 5                                                |
| **الأمان والتحقق** |  |  |  |  |
...
| BACKEND_MAIL_INVITE_SPACE_NAME_MAX_LENGTH  | الحد الأقصى لطول اسم المساحة أو المشروع في عناوين رسائل الدعوة. يؤدي تركه فارغًا أو ضبطه على 0 إلى تعطيل الاقتطاع | - | - | 32 |
| **تهيئة الجلسات/JWT** |  |  |  |  |
...
| TABLE_LIMIT_TABLES_PER_BASE_MAX            | الحد الأقصى لعدد الجداول لكل مشروع                                                   | 1000            | -        | 1000                                              |
...
| V2_TABLE_QUERY_OPS_ENABLED                 | ما إذا كانت عمليات استعلام الجداول مفعّلة؛ عند التعطيل لا تعرض صفحة **عمليات استعلام الجداول** أي بيانات. أعد التشغيل بعد التغيير        | false           | -        | true                                              |
| **التحليلات والمراقبة** |  |  |  |  |
...
| PUBLIC_DATABASE_PROXY                      | عنوان قاعدة البيانات المتاح خارجيًا (host:port) والمستخدم في بيانات الاعتماد التي تنشئها لوحة `اتصال قاعدة البيانات` في المشروع؛ اضبطه لتمكين وصول SQL خارجي للقراءة فقط | -               | -        | db.example.com:42345                              |
...
| BIG_TRANSACTION_TIMEOUT                    | مهلة المعاملات الداخلية الكبيرة (بالمللي ثانية)، مثل تصدير مشاريع كبيرة | 600000           | -        | 1200000                                           |
