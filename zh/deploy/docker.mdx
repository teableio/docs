...
生产环境部署请先阅读 [概览](/zh/deploy/architecture)，然后选择 [Kubernetes](/zh/deploy/k8s) 或云服务部署方案。
</Callout>
...
生产环境强烈推荐使用 **S3-compatible 对象存储**（S3 / MinIO / 其他兼容 S3 的服务），以获得更好的可靠性与扩展能力。
...
如果使用MinIO，请保证minio服务也启用了https, 并将BACKEND_STORAGE_MINIO_USE_SSL设置为true
<Tip>
