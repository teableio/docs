...
<AccordionGroup>
  <Accordion title="Pourquoi ai-je reçu une alerte d’utilisation des exécutions d’automatisation ?">
    ...
    La notification ouvre **Paramètres de l’Espace** → **Facturation**. Lorsqu’elles sont disponibles, elle indique quel workflow et quel Projet ont contribué au plus grand nombre d’exécutions. Les rappels de quota sont basés sur les exécutions comptabilisées dans la limite de votre forfait ; les vérifications d’utilisation inhabituelle peuvent également prendre en compte les exécutions récentes avec l’état **success**, **failed** ou **pending**.
