...
* **Targeted in-app announcements**: Admins can schedule localized announcements for selected users via banners, Toasts, modals, or sidebar cards, with retraction support.

* **Recycle Bin record snapshots**: View deleted records read-only and filter by resource type, operator, deletion time, creator, or creation time.

* **Record archiving for Business**: Browse, restore, permanently delete, and export archived records to CSV, with longer retention periods.

* **Improved published app management**: Quickly open apps, identify unpublished changes, and manage version history and rollbacks more intuitively.

## Fixes & Improvements

* **Improved table filtering stability**: Fixed errors when removing or clearing filters and opening views with incomplete date filter configurations.

* **Improved filter synchronization**: Toolbar summaries update immediately, while filter changes sync more reliably across connected clients.

* **Improved AI generation cancellation**: Stopping generation in chat, App Builder, or Agent workflows no longer causes server errors elsewhere.

* **Improved checkpoint generation stability**: Enhanced reliability under heavy loads, reducing state-saving failures in busy databases.

* **Improved large-table calculation performance**: Reduced write stalls, memory pressure, and 503 errors in large, highly connected tables.

* **Improved calculated field accuracy**: Fixed dependency propagation across comparisons, linked records, Lookup, Rollup, circular dependencies, and partial recalculations.

* **Faster Lookup updates**: Reduced Lookup propagation time after record updates, displaying linked data results sooner.

* **Optimized conditional Rollup calculations**: Avoided unnecessary broad recalculations in complex dependencies, improving responsiveness and reducing resource usage.

* **Improved field type conversion stability**: Fixed prolonged stalls when changing field types in tables containing calculated fields.

* **Consistent time handling**: Fixed time offsets in scheduled tasks, metadata updates, and database workflows for non-UTC deployments.

* **Improved AI workflow creation**: Generated nodes now appear near source nodes, and creation commands support folder selection.

* **Stronger sharing permissions**: Sharing controls now follow user and access token permissions, disabling unavailable options with explanations.

* **Enhanced field creation audits**: Field creation logs now include field names, types, and options for clearer schema tracking.

* **Fixed orphaned linked field deletion**: Linked fields can now be deleted after their referenced tables are removed.

* **Improved core data compatibility**: Enhanced reliability across formulas, Lookup, Rollup, filtering, sorting, grouping, search, aggregation, links, fields, and Base duplication.

* **Consistent empty-value writes**: Cleared supported fields now save as empty values, while required fields remain reliably validated.

* **Improved linked record write compatibility**: Link updates now accept single-link and multi-link formats across integrations, imports, and cardinality changes.

* **Improved date write validation**: Strict writes reject invalid dates, while type conversions save them as empty values.

* **Improved date field conversion**: Text-to-date and formula-to-date conversions skip invalid dates instead of blocking the entire conversion.

* **Fixed Rating field conversion**: Prevented invalid fragmented values and standardized decimal rounding for consistent ratings.

* **Stronger record and field permissions**: Table, row, and field permissions now apply consistently across record reads, updates, and deletions.

* **Fixed user avatar compatibility**: Updating users without custom avatars no longer fails, and initial-based default avatars display correctly.

* **Fixed grouped view loading**: Added required field information to prevent some grouped views from failing to open.

* **Improved view stability**: Enhanced reliability for creating, updating, sharing, importing, duplicating, live-updating, and collaboratively editing views.

* **Improved self-hosted user registration**: New users reliably join eligible auto-join Spaces as Viewers, including Spaces using SSO enrollment.

* **Improved unsubscribe page usability**: The page remains usable after preferences update successfully, even if subsequent refreshes fail.

* **Fixed duplicate automation execution**: Prevented action chains from running repeatedly and duplicating messages, Webhooks, or emails.

* **Improved formula field automation triggers**: Formula result changes now trigger automations correctly without duplicate runs or execution loops.

* **Improved bulk deletion undo and redo**: Large deletions can be restored or repeated more reliably, reducing errors and timeouts.

* **Optimized large imports and table duplication**: Reduced unnecessary initial history and database load without affecting regular editing history.

* **Improved mobile attachment previews**: Expanded spreadsheet viewing, improved horizontal scrolling, prevented control overlap, and correctly handled rotated images.

* **Faster Base loading and navigation**: Reduced duplicate requests and safely reopened eligible recent, pinned, or first-visit Bases directly.

* **Fixed invalid Base link navigation**: Invalid direct links now return to the standard entry flow without redirect loops or unnecessary errors.

* **Improved field validation messages**: Enhanced localized errors and identified duplicate fields during bulk and selection-based operations.

* **Stronger secret management**: Improved secret handling and secure credential rotation, reducing risks from long-lived credentials.

* **Optimized real-time grid loading and updates**: Reduced synchronization bandwidth and improved loading and responsiveness on slower networks.

* **Unified pricing information**: Removed outdated in-app pricing and FAQs, directing users to the official pricing page for current plan details.
